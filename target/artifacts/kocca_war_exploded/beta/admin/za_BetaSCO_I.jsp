...
function receiveMember(userid, name, resno, email, cono, jikwi, jikwinm, comp, compnm, tmp1, tmp2, tmp3){
	...
	function ZipFileUpload(){
		...
		p_cpseq= document.form1.p_cpseq.value;
		...
		document.form1.action = "/servlet/com.credu.scorm.xmlFileUpload?NextPage_URL=/learn/admin/contents/za_SCO_I.jsp&p_content_sync="+p_content_sync_1+"&p_cpseq="+p_cpseq;
	  ...
	  <table>	 
		<%	if(v_unzip_flag.equals("1")){	%>
		  ...
		  <td align=left width=100%>&nbsp;&nbsp;	
			  ...
			  <%=v_unzip_filename%> &nbsp;&nbsp;
			  동시업로드<input type=checkbox name="p_content_sync" >
			  <% } %>
		  </td>
		  </tr>
	  <% } else { %>
		 <tr>
		  ...
		  템플릿</b> : <a href = '/servlet/controller.library.DownloadServlet?p_savefile=imsmanifest.xml&p_realfile=imsmanifest.xml'>imsmanifest.xml</a> 
		  </td>
		  <td align=left width=10%>동시업로드<input type=checkbox name="p_content_sync" checked onclick=sco_change()></td>	  
		  </tr>
	  <% } %>
	  </table>
