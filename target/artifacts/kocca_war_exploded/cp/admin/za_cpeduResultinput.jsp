...
System.out.println("111111111111111111");
        for (i=2; i < sheet.getRows() ; i++ ) {
            ...
            if(isOK4*isOK5*isOK6 > 0){
	            ...
	            // data.setCono  (sheet.getCell(1,i).getContents()); // 사번
	            ...
	            data.setScoreFile(Double.valueOf(sheet.getCell(8,i).getContents()).doubleValue());	//총점 점수
	            ...
	            data.setAvetc1(0);
	            data.setAvetc2(0);
	            data.setIsgraduated(sheet.getCell(9,i).getContents());	//수료여부(Y/N)
	            //data.setIsgraduated("N");	//수료여부(Y/N)
	            data.setNotgraduetc(sheet.getCell(10,i).getContents());	//미이수사유(두자리코드)
	            data.setRank(sheet.getCell(14,i).getContents());	//성적우수자
                ...
                if(!sheet.getCell(9,i).getContents().equals(v_ispass)){
                	//실수료여부와 엑셀파일의 수료여부가 틀리면 붉은색으로 표시하여 알림.
                	v_ispass1 = "<font color=red>" + sheet.getCell(9,i).getContents() + "</font>";
                	v_ispass2 = "<font color=red>" + v_ispass + "</font>";
                	v_isErr4 = 0;
                }
                else{
                	v_ispass1 = sheet.getCell(9,i).getContents();
                	v_ispass2 = v_ispass;
                }
...
<%
        if (v_process.equals("insertFileToDB")) {
			...
			//외부교육과정결과 파일등록 정보저장
			isOK3 = cpResult.update_cpresult(connMgr, v_year, v_subj, v_subjseq, v_realFileName, v_newFileName);
...
<%		
        ...
        out.println("isOK_Final==>>"+isOK_Final);
        out.println("isOK3==>>"+isOK3);
