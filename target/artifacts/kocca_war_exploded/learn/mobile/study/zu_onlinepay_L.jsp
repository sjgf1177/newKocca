...
<script type="text/javascript" language="javascript">
	...
	function goPage(v_pageno)
	{
		...
		obj.p_process.value = "onlinePayList";
...
<div id="kocca_subContent">	
	...
	<form name="viewForm" method="post" action="/servlet/controller.mobile.course.CourseServlet">
		<input type="hidden" name="p_process" value="onlinePayList" />
		...
		<div class="onoff_tab">
			<ul>
				...
				<li class="tabmenu_l" style="width:50.8%;">
					<a title="오프라인과정" href="/servlet/controller.mobile.study.MyClassServlet?p_process=offlinePayList&p_menuid=<%=v_menuid%>"><span>오프라인과정</span></a>
				</li>
			</ul>
		</div>
	</nav>
...
%>		
		...
		<div><%=dbox.getString("d_goodname")%></div>
			<ul class="field_m">
				<li class="field_li"><span class="list_date01 color_red"><%=FormatDate.getFormatDate(dbox.getString("d_inputdate"), "yyyy.MM.dd")%> <%=NumberManager.getIntFormat(v_biyong)%>원</span><span class="color_blue">(<%=NumberManager.getIntFormat(v_price)%>원)</span></li>
