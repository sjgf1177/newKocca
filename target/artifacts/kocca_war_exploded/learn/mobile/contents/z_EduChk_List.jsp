...
%>
					<tr>
<%      if (i==0 && rowCnt>1) {  %>
						<td class="td_ct01" rowspan="<%= rowCnt %>"><span>토론</span></td>
<%      }else if (rowCnt==1) {  %>
                ...
                if(x.getRecordType().equals("STEP")){
                    ...
                    if(x.getIsbranch().equals("Y")){
                        ...
                        for(int j=0;j<brlist.size();j++){
                            ...
                            v_brlink=v_brlink + ""+bData.getSdesc()+"&nbsp;&nbsp";
                        }
                    }
...
%>
						<td class="td_lf01"><%= x.getLesson() + "-" + x.getSdesc() %></td>
						<td class="td_ct01"><%= get_isEducatedTxt(x.getIsEducated()) %></td>
						<td class="td_ct02"><%= FormatDate.getFormatDate(x.getLdate(),"yyyy/MM/dd-HH:mm") %></td>
