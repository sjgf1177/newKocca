...
//  2. 프로그램명 : za_CourseEnterFieldGubun_L.jsp
//  3. 개      요: 입과현황 - 분야별 회원구분별, 성별, 연령대별, 학력별, 문화산업종사자별, 비문화사업종사자별   현황
//  4. 환      경: JDK 1.5
//  5. 버      젼: 1.0
//  6. 작      성: 2009.12.10
//  7. 수      정:
...
function whenSelection(ss_action) {
	var process = 'selectEnterFieldGubunListPre';            //검색전
	...
	if (ss_action=="go")  {
		...
		//}
		top.ftop.setPam();
		process='selectEnterFieldGubunList';   //검색후
	}
...
function whenExcel() {
    ...
    document.form1.target = "_self";
    document.form1.action='/servlet/controller.course.CourseStateAdminServlet';
    document.form1.p_process.value = 'selectEnterFieldGubunListExcel';
...
function selectOption3(p) {
	var p_process;
	if( p == "NOR") {
		p_process = "selectEnterFieldListPre";
	} else {
		p_process = "selectEnterFieldGubunListPre";
	}
	document.form1.target = "_self";  
	document.form1.action = "/servlet/controller.course.CourseStateAdminServlet";
	document.form1.p_action.value = "change";
  ...
  <tr>
    <td align="center" valign="top">
                ...
                <td align="center">
                  <table border="0" cellspacing="0" cellpadding="0" width="99%" class="form_table">
                    ...
                    <tr>
                      ...
                      <td align="left" valign="middle">
		  		        <kocca_select:select name="s_option2" sqlNum="coursestatistics2"  param=" " onChange="selectOption1(this.value);" attr="통계구분2" selectedValue="selectEnterFieldListPre" isLoad="true" all="none" type="1" styleClass=""/>
				        &nbsp;
		  		        <kocca_select:select name="s_option3" sqlNum="coursestatistics3_1"  param=" " onChange="selectOption3(this.value);" attr="통계구분3" selectedValue="<%= ss_option3 %>" isLoad="true" all="none" type="1" styleClass=""/>
                      </td>
      ...
      <table width="97%" border="0" cellpadding="0" cellspacing="0">
        <tr>
          <td align="left" height="20">&nbsp;<b>분야별 <%= stat_text %> 입과현황</b></td>
          <td align="right">
      ...
      <table class="table_out" cellspacing="1" cellpadding="5">		
        <tr>
          <td colspan="<%= listGubun.size() * 2 + 4 %>" class="table_top_line"></td>
        </tr>
        <tr>
          <td class="table_title" width="5%" rowspan="3"><b>No</b></td>
          <td class="table_title" colspan="2"><b>구분</b></td>
          <td class="table_title" rowspan="3"><b>개설과목(개)</b></td>
          <td class="table_title" colspan="<%= listGubun.size() * 2 %>"><b>수강인원(명)</b></td>
        </tr>
        <tr>
          <td class="table_title" rowspan="2"><b>분야</b></td>
          <td class="table_title" rowspan="2"><b>난이도</b></td>
          <td class="table_title" colspan="<%= listGubun.size() %>"><b>분야별</b></td>
          <td class="table_title" colspan="<%= listGubun.size() %>"><b>난이도별</b></td>
        </tr>
        <tr>
<%
...
%>
          <td class="table_title"><b><%= dboxGubun.getString("d_codenm") %></b></td>
<%
			}
%>
<%
...
<%
			if (v_rowspan != 0) {
%>
          <td class="table_02_1" rowspan="<%= v_rowspan %>"><%= v_middleclassname %></td>
<%
			}
%>
...
<%
			if (v_rowspan != 0) {
				for (int j = 0; j < listGubun.size() ; j++) {
					DataBox dboxGubun = (DataBox)listGubun.get(j);
%>
          <td class="table_02_1" rowspan="<%= v_rowspan %>"><%= dbox.getInt("d_field_cnt"+(j+1)) %></td>
<%
			...
			for (int j = 0; j < listGubun.size() ; j++) {
				DataBox dboxGubun = (DataBox)listGubun.get(j);
%>
          <td class="table_02_1"><%= dbox.getInt("d_cnt"+(j+1)) %></td>
<%
...
<%
			for (int j = 0; j < listGubun.size() ; j++) {
				DataBox dboxGubun = (DataBox)listGubun.get(j);
%>
          <td class="table_02_3"><%= v_total_field_cnt[j] %></td>
<%
			}
			for (int j = 0; j < listGubun.size() ; j++) {
				DataBox dboxGubun = (DataBox)listGubun.get(j);
%>
          <td class="table_02_3"><%= v_total_cnt[j] %></td>
<%
			}
%>
        </tr>
