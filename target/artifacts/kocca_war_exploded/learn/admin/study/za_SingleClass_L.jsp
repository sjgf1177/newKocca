...
// 단일클래스 저장
function makeSingleClass() {
  if (getSingleClassCnt() < 1) {
    alert('단일클래스를 구성할 차수가 없습니다.');
    return;
  }
  document.form2.p_process.value = 'oneStopMakeClass';
  document.form2.p_action.value  = 'go';
  document.form2.submit();
}

// 구성할 차수 갯수
function getSingleClassCnt() {
  var cnt = 0;
  for(i=0;i<document.form2.length;i++) {
    if (document.form2.elements[i].name == 'p_tutors') {
      cnt++;
    }
  }
  return cnt;
...
function goPluralClassList() {
  document.form1.p_process.value = 'plurallistPage';
  document.form1.p_action.value  = 'go';
  document.form1.submit();
}

// 부강사 지정
function openSubTutor(p_subj, p_year, p_subjseq) {
...
%>
      ...
      <br>
      <br>
      <!----------------- 단일클래스구성, 복수클래스 구성, 단일클래스 저장버튼 시작 ----------------->
        ...
        <tr>
          <td width="125" height="23" align="center" valign="middle">
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
              ...
              <tr>
                <td background="/images/admin/student/student_blue_img5.gif" align="center" height="16" valign="middle"><b><font color="#FFFFFF">단일클래스구성</font></b></td>
              </tr>
              <tr>
          ...
          <td width="125">
            <table width="100%" border="0" cellspacing="0" cellpadding="0">
              ...
              <tr>
                <td background="/images/admin/student/student_black_img5.gif" align="center" height="16" valign="middle"><b><font color="#FFFFFF"><a href="javascript:goPluralClassList()" class="c">복수클래스구성</a></font></b></td>
              </tr>
              <tr>
          ...
          <td align="right" valign="top">
            <a href="javascript:makeSingleClass()"><img src="/images/admin/button/btn_singleclass.gif" border="0"></a>&nbsp;
            <a href="javascript:insertFileToDB()"><img src="/images/admin/button/btn_fileplus.gif" border="0"></a>
          </td>
        </tr>
      </table>

      <!----------------- 단일클래스구성, 복수클래스 구성, 단일클래스 저장버튼 끝 ----------------->
      ...
      <form name="form2" method="post" action="/servlet/controller.study.ClassServlet">
        ...
        <tr>
          ...
          <td class="table_title"><b>강사선택</b></td>
          <td class="table_title"><b>부강사</b></td>
          <td class="table_title"><b>부강사 지정</b></td>
        </tr>
...
%>
            <tr>
              ...
              <td class="table_02_1">
<%          if (data.getClasscnt() < 2) { %>
                <select name="p_tutors">
<%
                tutor = data.getTutor();
                if (tutor != null) {
                    v_tutorsize = tutor.size();
                    for (int j=0; j<tutor.size(); j++) {
                        tutordata = (TutorSelectData)tutor.get(j);
%>
                  <option value="<%=data.getSubj()+";"+data.getYear()+";"+data.getSubjseq()+";"+tutordata.getUserid()%>"<% if (tutordata.isSelected()) { %> selected<% } %>><%=tutordata.getName()%></option>
<%
                    }
                } else {
                    v_tutorsize = 0;
...
<%          }    %>
              ...
              <td class="table_02_1"><% if (data.getClasscnt() < 2) { %><%=data.getSubtutor()%><% } %></td>
              <td class="table_02_1"><% if (data.getClasscnt() < 2 && v_tutorsize > 1 ) { %><a href="javascript:openSubTutor('<%=data.getSubj()%>','<%=data.getYear()%>','<%=data.getSubjseq()%>')"><img src="/images/admin/button/b_jijung.gif" border="0"></a><% } %></td>
            </tr>
<%

            //과정수에 따라서 나머지 속해있는 과정을 출력.
            for( int k = 0;  k < ( v_subjcnt -1 ); k++ ){
                i++;
                data  = (ClassListData)list.get(i);
%>
...
%>
            ...
            <tr>
              <td class="table_02_2" colspan="2"><%=data.getSubjnm()%></td>
              <td class="table_02_1" rowspan="<%=data.getSubjcnt()%>"><%=StringManager.cutZero(data.getSubjseqgr())%></td>
              <td class="table_02_1"><%=FormatDate.getFormatDate(data.getEdustart(),"yyyy/MM/dd")%>~<%=FormatDate.getFormatDate(data.getEduend(),"yyyy/MM/dd")%></td>
              <td class="table_02_1"><%=data.getStudentcnt()%></td>
              <td class="table_02_1"><%=data.getClasscnt()%></td>
              <td class="table_02_1"><%=data.getNoassignstudentcnt()%></td>
...
<form name="form3" method="post" action="/servlet/controller.study.ClassServlet">
  ...
  <input type="hidden" name="p_call_url"   value="za_SingleClass_L">
</form>
</table>
