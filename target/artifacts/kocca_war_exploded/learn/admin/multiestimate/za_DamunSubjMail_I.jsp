<%
/**
 * file name : za_DamunSubjMail_I.jsp
 * date      : 2003/08/20
 * programmer:
...
<%
    for(int i=0;i<v_checks.size();i++) {              
      ...
      st = new StringTokenizer(v_schecks,"|");
		...
		if(st.hasMoreTokens())   v_gubuns = (String)st.nextToken();
		if(st.hasMoreTokens())   v_subjuserids = (String)st.nextToken();
        if(st.hasMoreTokens())   v_obuserids = (String)st.nextToken();
