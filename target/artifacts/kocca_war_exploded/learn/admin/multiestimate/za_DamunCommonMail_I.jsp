...
<%
    ...
    Vector v_checks = null;
    Vector v_pppchecks = (Vector)request.getAttribute("DamunMailSendList");
	if(v_pppchecks.size()>0){
	    	v_checks = v_pppchecks;
	}else{
...
function previewPage(){
 ...
 if (realsize(document.form2.p_title.value) > 200) {
    ...
    document.form2.p_process.value = 'DamunMailPreviewPage';
...
<%
    for(int i=0;i<v_checks.size();i++) {              
      ...
      st = new StringTokenizer(v_schecks,"|");
		if(st.hasMoreTokens()) v_gubuns = (String)st.nextToken();//System.out.println(v_gubuns);
		if(st.hasMoreTokens()) v_subjuserids = (String)st.nextToken();//System.out.println(v_subjuserids);
        if(st.hasMoreTokens()) v_obuserids = (String)st.nextToken();//System.out.println(v_obuserids);
        if(st.hasMoreTokens()) v_names = (String)st.nextToken();//System.out.println(v_names);
%>
