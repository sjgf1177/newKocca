...
<% if (StringManager.substring(box.getSession("gadmin"), 0, 1).equals("A")) {
        SelectSubjBean selectbean = new SelectSubjBean ();
        ArrayList list = selectbean.getOnlyUpperClass(box);
        DataBox  selectdbox = null;
%>
        <tr>
          <td class="table_title"><b>대분류</b></td>
          <td class="table_02_2">
            <select name= "p_upperclass_i">
<%
        for (int k = 0; k<list.size(); k++) {
            selectdbox = (DataBox)list.get(k);
            out.println("           <option value='" + selectdbox.getString("d_upperclass") + "'>" + selectdbox.getString("d_classname") + "</option>");
        }
%>
            </select>
          </td>
...
<% } else  {                                                   %>
            <input type="hidden" name="p_upperclass_i"   value="<%=box.getSession("upperclass")%>">
<% }                                                           %>
...
-->
      ...
      <table width="11%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td align="center"><a href="javascript:InsertCourse()"><img src="/images/admin/button/btn_save.gif" border="0"></a></td>
