<%
/**
 * file name : za_SulmunSubjUser_L.jsp
 * date      : 2003/09/12
 * programmer:
 * function  : 설문 응시
 */
...
function whenSubmit(){
  ...
  for(i=0;i<document.form3.length;i++) {
    ...
    } else if (document.form3.elements[i].type=="radio" && document.form3.elements[i].type=="textarea") {
      b_type="radio";
      if (document.form3.elements[i].checked==true) {
        temp = temp+c_value+':';
      }
...
<table cellspacing="0" cellpadding="0" class="open_board_table_out1" align="center">
  <tr> 
   ...
   <tr> 
    <td width="13">&nbsp;</td>
    <td><b>설문지명 : <%=paperdata.getString("d_sulpapernm")%> / 설문지번호 : <%=v_sulpapernum%> / 문제수: <%=paperdata.getInt("d_totcnt")%></b></td>
  </tr>
  <tr> 
    <td height="5"></td>
  </tr>
...
<form name="form2" method="post" action="/servlet/controller.research.SulmunSubjResultServlet">
  ...
  <tr> 
    <td> 
      ...
      <form name="form3" method="post" action="/servlet/controller.research.SulmunSubjResultServlet">
<% 
...
%>
        <tr> 
          <td class="board_text4"> 
            <input type="radio" name="<%=dbox.getInt("d_sulnum")%>" value="">기타<textarea name="<%=dbox.getInt("d_sulnum")%>" cols="90" rows="2" class="input"></td>
        </tr>
<%	
...
%>  
	  <a href="javascript:whenSubmit()"><img src="/images/user/study/presentation_butt1.gif" width="54" height="20" border="0"></a>
<%
    }
%>  
    </td>
