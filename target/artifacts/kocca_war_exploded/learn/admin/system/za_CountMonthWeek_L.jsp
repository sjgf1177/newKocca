...
//**********************************************************
//  1. 제      목: 접속통계
//  2. 프로그램명 : za_CountMonthWeek_L.jsp
//  3. 개      요: 접속통계 - 월요일통계 리스트
//  4. 환      경: JDK 1.3
//  5. 버      젼: 1.0
//  6. 작      성: 정상진 2003. 7. 8
//  7. 수      정:
...
<%
    ...
    String v_week_view[]={"일","월","화","수","목","금","토"};
...
<!--
	...
	function whenSelection(){
        document.form1.action = "/servlet/controller.system.CountServlet";
        document.form1.p_process.value = "selectMonthWeek";
        document.form1.submit();
	}
  ...
  <tr>
    <td align="center" valign="top">
      ...
      <!----------------- 검색조건 form 끝 ----------------->
        ...
        <table width="97%" border="0" cellspacing="0" cellpadding="0">
          <tr>
            <td align="center"><font face="굴림" size="3.5">◈<b>접속통계 (월-요일 통계) <%=v_year%>년 Total :<%=v_yearcount%></font></b></td>
          </tr>
        </table>
        <br>
        ...
        <table width="37%" border="0" cellpadding="0" cellspacing="0">
          <tr>
            ...
            <td align="center"><img src="/images/admin/system/statistics4_butt.gif" width="76" height="20" border="0"></td>
          </tr>
...
%>
        ...
        <table width="97%" border="0" cellspacing="1" cellpadding="0">
          ...
          <tr>
            ...
            <td class="table_title"><b>[<%=v_year%>년 <%=v_month%>월]  요일별 접속자수입니다.</b></td>
          </tr>
...
<%
            ...
            for(int i = 1; i <= 7; i++) {
              ...
              if (j < list2.size()) {
                ...
                v_date_week   = data3.getDate_week();
                v_cnt         = data3.getCnt();
                v_percent     = (v_cnt*90)/v_maxcnt;
                if (v_percent ==0) v_percent     = 1;       // 0%일경우 그래프가 틀려짐
                j++;
              } else {
                v_date_week    = String.valueOf(i);
                v_cnt         = 0;
                v_percent     = 1;
              }


              while (StringManager.toInt(v_date_week)  > i) {
%>

          <tr>
            <td height="25" align="center" bgcolor="#EEEEEE"><%=v_week_view[i-1]%>요일</td>
            <td bgcolor="#F7F7F7">
...
%>
          <tr>
            <td height="25" align="center" bgcolor="#EEEEEE"><%=v_week_view[Integer.parseInt(v_date_week)-1]%>요일</td>
            <td bgcolor="#F7F7F7">
              <table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
