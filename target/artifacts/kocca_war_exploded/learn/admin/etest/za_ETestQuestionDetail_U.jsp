<%
/**
 * file name : za_ExamQuestionDetail_U.jsp
 * date      : 2003/09/01
 * programmer:
 * function  : 평가 문항관리 - 수정페이지
 */
%>
...
<%
    ...
    String  ss_subj    = box.getString("s_subj");//System.out.println("ss_subj " + ss_subj);
    int     v_etestnum = box.getInt("p_etestnum");//    System.out.println("v_etestnum " + v_etestnum);
...
function openExamText() {
  if (document.form3.s_subj.value == '') {
    alert('과정코드를 선택하세요');
    return;
  }
  ...
  document.form3.target = "openExamText";
  document.form3.action = "/servlet/controller.etest.ETestServlet";
  document.form3.p_process.value = "QuestionTextListPage";
...
<% } else { %>
            <a href="javascript:UpdateQuestion()"><img src="/images/admin/research/save1_butt.gif" width="37" height="18" border="0"></a>
            <a href="javascript:DeleteQuestion()"><img src="/images/admin/research/del1_butt.gif" width="37" height="18" border="0"></a>
<% } %>
          </td>
        </tr>
        <tr>
	...
	<form name="form2" method="post" enctype="multipart/form-data" >
        ...
        <tr>
          ...
          <td width="30%" class="table_02_2">
            <%= ETestBean.getCodenm(false, false, "s_levels", ETestBean.ETEST_LEVEL, dbox0.getString("d_levels"))%>
            <% //-- getCodenm(isChange, isALL, selectbox name, gubun code, selected code) ---%>
          </td>
          <td width="15%" class="table_title_01">문제분류</td>
          <td width="30%" class="table_02_2">
            <%= ETestBean.getCodenm(false, false, "s_etesttype", ETestBean.ETEST_TYPE, dbox0.getString("d_etesttype"))%>
          </td>
        </tr>
        ...
        <tr>
          ...
          <td width="30%" class="table_02_2">
            <table cellspacing="0" cellpadding="0" class="s_table">
              <tr>
                <td width="167">
                  <input name="p_addseq" type="text" class="input" size="25" value="<%=String.valueOf(dbox0.getString("d_addseq"))%>" readonly>
                </td>
                    ...
                    <tr>
                      <td rowspan="9" width="50%" class="table_02_1">
                        <table cellspacing="0" cellpadding="0" class="s_table">
                          <tr>
                            <td>
                              <textarea name="p_text" cols="70" rows="7" class="input"><%=dbox0.getString("d_text")%></textarea>
                            </td>
                          </tr>
                          <tr>
                            <td height="5"></td>
...
<%  
    for (int k=1; k <= 9; k++) {
        if(list.size() > k || list.size() == k) {
        DataBox dbox = (DataBox)list.get(k-1);   
        %>
                    
                      <td class="table_02_1"><%=String.valueOf(k)%></td>
                      <td class="table_02_1">
                        <input name="p_seltext<%=k%>" type="text" class="input" size="50" value=`<%=StringManager.replace(dbox.getString("d_seltext"), "`", "'")%>`>
                      </td>
                      <td class="table_02_1">
                        <table cellspacing="0" cellpadding="0" class="s_table">
                          <tr>
                            <td width="21%">
                              <input type="checkbox" name="p_isanswer<%=k%>" value="<%=dbox.getString("d_isanswer")%>" <% if(dbox.getString("d_isanswer").equals("Y")) { %>  checked<% } %>>
                            </td>
                            <td>정답여부[<%=dbox.getString("d_isanswer")%>]</td>
                          </tr>
                        </table>
                      </td>
                    </tr><tr>
        ...
        <tr>
          ...
          <td colspan="3" class="table_02_2">
            <table cellspacing="0" cellpadding="0" class="s_table">
              <tr>
                <td width="100%">
                  <input name="p_image" type="FILE" class="input" size="40">&nbsp;<%=dbox0.getString("d_realimage")%>
                </td>
              </tr>
            </table>
          </td>
        ...
        <tr>
          ...
          <td colspan="3" class="table_02_2">
            <table cellspacing="0" cellpadding="0" class="s_table">
              <tr>
                <td width="100%">
                  <input name="p_audio" type="FILE" class="input" size="40">&nbsp;<%=dbox0.getString("d_realaudio")%>
                </td>
              </tr>
            </table>
          </td>
        ...
        <tr>
          ...
          <td colspan="3" class="table_02_2">
            <table cellspacing="0" cellpadding="0" class="s_table">
              <tr>
                <td width="100%">
                  <input name="p_motion" type="FILE" class="input" size="40">&nbsp;<%=dbox0.getString("d_realmotion")%>
                </td>
              </tr>
            </table>
          </td>
