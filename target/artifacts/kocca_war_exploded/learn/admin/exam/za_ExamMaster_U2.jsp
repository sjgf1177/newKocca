<%
/**
 * file name : za_ExamMaster_U2.jsp
 * date      : 2003/09/03
 * programmer:
 * function  : 평가마스터 수정
 */
...
<%
	...
	System.out.println("v_startdt:"+v_startdt);
	System.out.println("v_enddt:"+v_enddt);

	int s_lesson = 0;
	int s_levels = 0;
	int s_type = 0;

	DataBox dbox0 = (DataBox)request.getAttribute("ExamMasterData");
%>
<html>
<head>
<title>평가마스터수정</title>
...
%>
				<tr>
				   ...
				   <td>
				       (<input name="p_cntlevelM<%=i+1+istlesson%>M<%=j+1%>M<%=k+1%>" type="text" class="input" size="3" value="<%=examcnt%>">&nbsp;/ <%=dbox.getInt("d_levelscount")%>)
...
%>
      ...
      <table  border="0" cellspacing="0" cellpadding="0">
        <tr> 
		  ...
		  <td width=8></td>
           <td><a href="javascript:UpdateMaster('<%=s_lesson%>','<%=s_levels%>','<%=s_type%>')"><img src="/images/admin/button/btn_apply.gif"  border="0"></a></td>
		   <td width=8></td> 
...
<script language="JavaScript">
<!--
function UpdateMaster(s_lesson, s_levels, s_type) {
  if (!chkData()) {
    return;
  }
  document.form1.p_process.value = 'ExamMasterUpdate';
