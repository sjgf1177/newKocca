              ...
              <tr>
                <td align="left" width="99%" >&nbsp;&nbsp;
                  <b>콘텐츠제목:</b>  <b><font color=blue><%= d.getScoTitle()%></font></b>&nbsp;&nbsp;&nbsp;&nbsp;
					...
					<script language='javascript'>object_ALEX_FTP('<%= conf.getProperty("ftp.server.ip")%>', '<%=conf.getProperty("ftp.server.id")%>', '<%= conf.getProperty("ftp.server.pw")%>','<%=d.getScolocate()%>' );</script>
...
<% if ((d.getHighoid() != "" ) && (d.getMetalocation() != "")) { %>
           ...
           <% if( d.getMetadata() != 0 ) {  %>
		   <td class="table_03_1">
           <a href="javascript:Meta_Add('<%=d.getOid()%>','<%=d.getMetalocation()%>','<%=d.getScolocate()%>')"><img src=/images/admin/button/b_view.gif border=0></a></td>         
		   <% } else { %>
		    <td class="table_03_1">
           <a href="javascript:Meta_Add('<%=d.getOid()%>','<%=d.getMetalocation()%>','<%=d.getScolocate()%>')"><img src=/images/admin/button/b_regist.gif border=0></a></td> 
		   <% } %>
<% } else { %>
           <td class="table_03_1"> - 
           </td>         
