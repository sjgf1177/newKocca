<%
/**
 * file name : za_JindanQuestion_U3.jsp
 * date      : 
 * programmer:
 * function  : 진단테스트 문제관리 OX식 - 수정페이지
 */
%>
...
<%
	...
	int v_examnum = dbox0.getInt("d_jindannum");
	String v_examtype = box.getStringDefault("d_jindantype", dbox0.getString("d_examtype"));
...
function chkData() {
  ...
  var v_examtype = document.form2.d_jindantype.value;
  var v_selcount = 0;
  var v_seltextcnt = 0;
  var v_test      = '';
     ...
     <br>
      ...
      <table cellpadding="0" cellspacing="0" class="table1">
        <tr>
		  ...
		  [<%=GetCodenm.get_subjnm(v_subj)%>] [<%=dbox0.getString("d_jindannum")%>번] 문제 수정</td>
        </tr>
        <tr>
          <td height="3"></td>
        </tr>
      ...
      <form name="form2" method="post" enctype="multipart/form-data">
		...
		<tr>
		  ...
		  <SELECT name="d_jindantype" onchange="display();"> 
            <option value=1>객관식</option> 
            <option value=2>주관식</option> 
            <option value=3>OX식</option>  
			<script>document.form2.d_jindantype.value = <%=v_examtype%>;</script>
          </SELECT> 
          </td>
		</tr>
		<tr>
    	  <td class="table_title">난이도</td>
          <td class="table_02_1" colspan="3" align="left"><%=CodeConfigBean.getCodeGubunSelect(JindanBean.JINDAN_LEVEL,"", 1, "p_levels", dbox0.getString("d_levels"), "", 0)%>
		  </td>
		</tr>
		<tr>
...
%>
        <tr>
          ...
          <td class="table_02_1">
             <input name="p_seltext<%=String.valueOf(k)%>" type="text" class="input2" size="3" value=`<%=StringManager.replace(dbox.getString("d_seltext"), "`", "'")%>` readonly>
          </td>
          <td class="table_02_1" width="30%">
