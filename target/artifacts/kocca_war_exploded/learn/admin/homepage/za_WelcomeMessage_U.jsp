...
<%
	...
	if (dbox != null) {
	    ...
	    v_startdate         = FormatDate.getFormatDate(dbox.getString("d_startdate"), "yyyy-MM-dd");
	    v_enddate           = FormatDate.getFormatDate(dbox.getString("d_enddate"), "yyyy-MM-dd");
	    v_month             = dbox.getString("d_month");
	    v_date              = dbox.getString("d_ldate");
	    v_name              = dbox.getString("d_name");
	    
	    if (v_type.equals("B1") && v_month.length() == 6 ) {
		    v_year     = v_month.substring(0, 4);
		    v_month    = v_month.substring(5, 6);
	    }
	 }  
...
<body bgcolor="#FFFFFF" text="#000000" topmargin="0" leftmargin="0" onload="selectType('<%=v_type %>');">
      ...
      <table width="97%" border="0" cellspacing="0" cellpadding="0" class=page_title>
        ...
        <table class="table_out" cellspacing="1" cellpadding="5">
          ...
          <tr>
            ...
            <td colspan="3" height="25" class="table_02_2">
              <table>
                <tr>
                  <td width="40%">
                    <input type="radio" name="p_type" value="A1" <%=v_type.equals("A1") ? "checked" : "" %>  onclick="selectType('A1');"/>기간별 설정
                  </td>
                  <td>
                    <input name="p_startdate" id="p_startdate" type="text" class="datepicker_input1" size="10" value="<%=v_startdate%>"> 일 ~
                    <input name="p_enddate" id="p_enddate" type="text" class="datepicker_input1" size="10" value="<%=v_enddate%>"> 일
                  </td>  
                </tr>
                <tr>
                  <td width="40%">
                    <input type="radio" name="p_type" value="B1" <%=v_type.equals("B1") ? "checked" : "" %>  onclick="selectType('B1');">월별 설정
                  </td>
                  <td>
                    <select name="p_year">
                        ...
                        if (v_type.equals("B1")){
	                        int ss_year = Integer.parseInt(v_year);
	                        
	                        if( ss_year >= currentYear) {
	                        	for(int i = 0 ; i < 2 ; i++) {
	                        %>
	                            <option value="<%=currentYear +i%>" <%=currentYear +i == ss_year ? "selected" : "" %>><%=currentYear +i %></option>
	                        <% }%>  
	                        <%
	                        } else {
	                    	   for(int i = ss_year ; i < currentYear+1 ; i++) {
	                        %>
	                            <option value="<%=currentYear +i%>" <%=currentYear +i == ss_year ? "selected" : "" %>><%=currentYear +i %></option>
	                        <% }
	                        }
                    ...
                    <select name="p_month">
                      <option value ="">선택</option>
                      <%  for(int i = 1 ; i <= 12 ; i++) { %>
                      <option value = "<%=i < 10 ? "0"+i : i  %>" <%= v_type.equals("B1") && v_month.equals(""+i) ?  "selected"  : "" %>><%=i %>월</option>  
                      <%  }%> 
                    </select>
                  </td>
                </tr>
          ...
          <tr>
            ...
            <td colspan="3" height="25" class="table_02_2">
                <textarea name="p_content" style="width:100%;height:50px"><%=v_content %></textarea>
            </td>
          </tr>
        ...
        <table  border="0" cellspacing="0" cellpadding="0">
          <tr> 
            ...
            <td width=4></td>
            <td align="center"><a href="javascript:deleteMessage()"><img src="/images/admin/button/btn_del.gif" border="0"></a></td>
            <td width=4></td>
