...
<!--
    ...
    function selectMember(userid, name, resno, email, registgubun, comptext, tmp1, tmp2, tmp3) {
        opener.receiveMember(userid, name, resno, email, registgubun, comptext, tmp1, tmp2, tmp3);
        self.close();
    }
    ...
    function search() {
        if (document.form1.p_key1.value == "") {
         ...
         document.form1.p_process.value = "member";
	  ...
	  <br>
            ...
            <table width="510" border="0" cellspacing="1" cellpadding="0">
              ...
              <tr>
                ...
                <td width="20%" class="table_title"><b>ID</b></td>
                <td width="20%" class="table_title"><b>성명</b></td>
                <td width="20%" class="table_title"><b>소속</b></td>
                <td width="20%" class="table_title"><b>직업</b></td>
                <td width="20%" class="table_title"><b>가입경로</b></td>
              </tr>
<%
         int i = 0;;
            ...
            for(i = 0; i < list.size(); i++) {
                ...
                v_comptext		= data.getTmp3();  // 소속
                v_registgubun   = data.getTmp4();  // 가입경로
				v_comptel		= data.getTmp5();	
				v_jikup			= data.getTmp6();
                v_dispnum		= data.getDispnum();
                v_totalpage		= data.getTotalpagecount();

%>
              <tr>
                <td height="26" class="table_01"> <input type="radio" name="no" value="<%=v_userid%>" onClick="javascipt:selectMember('<%=v_userid%>' ,'<%=v_name%>' ,'<%=v_resno%>' ,'<%=v_email%>' ,'<%=v_registgubun%>' ,'<%=v_comptext%>' , '<%=v_key2%>', '', '<%=v_comptel%>')"></td>
                <td class="table_02_1"><%=(i+1)%></td>
                <td class="table_02_1"><%=v_userid%></td>
                <td class="table_02_1"><p><%=v_name%></p></td>
                <td class="table_02_1"><%=v_comptext%></td>
                <td class="table_02_1"><%=v_jikup%></td>
                <td class="table_02_1"><%=v_registgubun%></td>
              </tr>
<%
          }
%>
