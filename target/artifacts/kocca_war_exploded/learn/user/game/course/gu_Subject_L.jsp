...
<% if(v_tem_grcode.equals("N000010") && !v_gubun.equals("4")) {	//교원연수 %>
	window.onload = function () { whenSelect('<%=v_iscourseYn%>','4'); }
<% } %>
//-->
</SCRIPT>
...
<% } %></td>
				...
				<tr>
					<td colspan="2"><img src="/images/user/game/apply/type1/apply02_bar.gif" vsapce="5"></td>
				</tr>
			</table>
...
%>
				<tr align="center" valign="bottom" style="padding:5px 0px 4px 0px;">
					...
					<!--<td style="color:black;"><%//=v_sul_avg%>&nbsp;<span style="color:#e7bd0f;"><%=v_imgStart%></span></td>-->
					<td><button onClick="javascript:fnInterestInsert('<%=v_subj%>','<%=v_subjnm%>','<%=v_isonoff%>','<%=v_scupperclass%>','<%=v_uclassnm%>')">관심과정담기</button></td>
