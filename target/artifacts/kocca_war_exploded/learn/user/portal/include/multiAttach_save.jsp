...
<%
    ...
    String  sUploadTempPath = uploadUtil.getTempUploadDir(sServletName);
    String  sWebUploadTempPath = uploadUtil.getWebTempUploadDir(sServletName); 
    
    String  UplodFileExt    = sUploadNm.substring(sUploadNm.lastIndexOf(".")+1);
    System.out.println("UplodFileExt : "+UplodFileExt);
    ...
    try {
        ...
        while (iter.hasNext())
        {
            ...
            if (!item.isFormField())
            {
                ...
                {
                    if ( ((String)sExtArr.get(i)).toLowerCase().equals(sExt) )
                    {
                        bResultExt  = true;
                        break;
                    }
                ...
                //파일형식 제한 : UploadUtil.java 에서 필터링 (gif, jpg, bmp, png)
                if ( UplodFileExt.equals("NOT_EXT"))
                {
                    uploadedFile.delete();
...
%>
  ...
  <!--
    ...
    parent.setUploadFile(attach_obj);
