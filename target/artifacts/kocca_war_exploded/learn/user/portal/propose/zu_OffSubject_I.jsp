...
<script language="JavaScript"><!--
    ...
    //교육정원 및 대기자 상관없이 모든 수강생 수강신청 받음 - 2014.01.15 by cjk
    //임호수과장, 구연진 과장님 요청으로 인하여 수정함
...
function whenPropose() {
    ...
    if( document.form1.hp1.value == "" || document.form1.hp2.value == "" || document.form1.hp3.value == ""){
        alert("휴대전화를 입력하세요.");
        if(document.form1.hp1.value == ""){
            $("#hp1").focus();
        }else if(document.form1.hp2.value == ""){
            $("#hp2").focus();
        }else if(document.form1.hp3.value == ""){
            $("#hp3").focus();
        }
        return;
    }
    ...
    if( document.form1.p_jikchaeknm.value == "" ) {
        alert("직책을 입력하세요.");
        document.form1.p_jikchaeknm.focus();
        $("#p_jikchaeknm").focus();
        return;
    }
    ...
    if(f.p_selfintro){
        ...
        if(r<300){
            alert("현재 자기소개(경력사항)는 = "+ r + "Byte 입니다.\n자기소개는 300Byte 이상 입력해 주시기 바랍니다.");
            $("#p_selfintro").focus();
            return;
        }

        if(r>3000){
            alert("현재 자기소개(경력사항)는 = "+ r + "Byte 입니다.\n자기소개는 3000Byte를 넘을수 없습니다.");
            $("#p_selfintro").focus();
            return;
        }
        ...
        if(d<300){
            alert("현재 지원동기는 = "+ r + " 자 입니다.\n지원동기는 300Byte 이상 입력해 주시기 바랍니다.");
            $("#p_support").focus();
            return;
        }
        if(d>3000){
...
<%
    if(box.getString("p_subj").equals("SB13033")) {
%>

    if(document.form1.p_attfile.value == ""){
        alert("첨부파일을 등록해 주세요.");
        return;
    }
...
%>
    ...
    if(!confirm("<%= dbox.get("d_subjnm") %>과정을 수강신청하시겠습니까?")) {
         return;
    }
...
<form name="form1" action="/servlet/controller.off.ProposeOffServlet" method="post" enctype="multipart/form-data">
    ...
    <input type="hidden" name="p_apply_name" id="oApplyName" />
    <input type="hidden" name="p_apply_session" id="oApplySesson" value="" />
    <input type="hidden" name="p_apply_belong_title" id="oApplyBelongTitle" value="" />

    <input type="hidden" name="p_nickname" id="oNickname" />
    <input type="hidden" name="p_private_sns" id="oPrivateSns" value="" />
    ...
    <div id="pop_container">
        <div id="contentwrap">
            <dl class="guide_list">
                ...
                <dd>교육지원서에 의해 교육선발의 적합성이 판단되므로 성실한 기재를 바랍니다.</dd>
                <dd>지원서의 허위기재, 기재착오 등은 교육참여 부적격 사유로 불이익을 받으실 수 있습니다.</dd>
            </dl>
...
%>
            <table width="100%" cellpadding="0" cellspacing="0" border="0">
                <tr>
                    <td width="100px"><h3 class="pop_tit"><img src="/images/portal/common/stit_applystep2.gif" alt="신청과정" /></h3></td>
                    <%--<td>--%>
            ...
            <tr>
                ...
                <td style="padding-left:10px;padding-right:0" class="in">
                    ...
                    <input type="text" id="hp1" name="hp1" onchange="$('#p_handphone').val($('#hp1').val() + '-' + $('#hp2').val() + '-' + $('#hp3').val());"
                        class="inbox" style="width:38px;" value="<%= hp[0] == null ? "" : hp[0] %>" maxlength="3" />-
                    <input type="text" id="hp2" name="hp2" onchange="$('#p_handphone').val($('#hp1').val() + '-' + $('#hp2').val() + '-' + $('#hp3').val());"
                        class="inbox" style="width:38px;" value="<%= hp[1] == null ? "" : hp[1] %>" maxlength="4" />-
                    <input type="text" id="hp3" name="hp3" onchange="$('#p_handphone').val($('#hp1').val() + '-' + $('#hp2').val() + '-' + $('#hp3').val());"
                        class="inbox" style="width:38px;" value="<%= hp[2] == null ? "" : hp[2] %>" maxlength="4" />
                </td>
            ...
            <tr>
                ...
                <td style="padding-left:10px;padding-right:0">
                    <input type="text" id="p_comptext" name="p_comptext" value="<%= dbox.getString("d_comptext") %>" class="inbox"><br/>
                    * 학생:소속학교, 직장인:소속회사,<br/>&nbsp;&nbsp;기타:직업
                </td>

<%
        if (box.get("p_subj").equals("SK12016")) {
%>
                <th>&nbsp;</th><td>&nbsp;</td>
<%
        } else {
...
%>
            ...
            <tr>
                <th style="padding-left:0;padding-right:0;text-align:center;">직책</th>
                <td style="padding-left:10px;padding-right:0" class="in end"><input type="text" id="p_jikchaeknm" name="p_jikchaeknm" class="inbox" style="width:162px;" value="<%= dbox.get("d_jikchaeknm") %>" /></td>
                <th style="padding-left:0;padding-right:0;text-align:center;">소속부서</th>
                <td style="padding-left:10px;padding-right:0" class="in end"><input type="text" id="p_deptnam" name="p_deptnam" class="inbox" style="width:162px;" value="<%= dbox.get("d_deptnam") %>" /></td>
            </tr>
<%
...
%>
            <tr>
                ...
                <td style="padding-left:10px;padding-right:0" colspan="3" class="end"><textarea rows="5" cols="90" name="p_intro" id="p_selfintro" onkeyup="textareaLength('p_intro','p_selfintroLbael');"><%= dbox.get("d_intro") %></textarea>
                    <!-- * 최소 300Byte, 최대 3000Byte 작성 바랍니다. (한글,특수문자 2byte 나머지 1Byte)-->
                    <br/>
                    * 최대 3000Byte까지 작성 가능합니다. (한글,특수문자 2byte 나머지 1Byte)
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;현재 Byte : <label id="p_selfintroLbael" style="font-weight:bold;"></label>
                </td>
            </tr>
            <tr>
                <th style="padding-left:0;padding-right:0;text-align:center;">지원동기</th>
                <td style="padding-left:10px;padding-right:0" colspan="3" class="end"><textarea rows="5" cols="90" name="p_motive" id="p_support" onkeyup="textareaLength('p_motive','p_motiveLbael');"><%= dbox.get("d_motive") %></textarea>
                    <!-- * 최소 300Byte, 최대 3000Byte 작성 바랍니다. (한글,특수문자 2byte 나머지 1Byte)-->
                    </br>
                    * 최대 3000Byte까지 작성 가능합니다. (한글,특수문자 2byte 나머지 1Byte)
                    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;현재 Byte : <label id="p_motiveLbael" style="font-weight:bold;"></label>
                </td>
            </tr>
<%
        }
...
%>
            <tr>
                ...
                <td style="padding-left:10px;padding-right:0" colspan="3" class="end"><input type="file" name="p_attfile" class="inbox"/></td>
            </tr>
<%
        }
%>
...
%>
            ...
            <tr>
                ...
                <td colspan="3" class="end">
                    <input type="text" class="inbox" style="width:200px;" name="p_apply_name_input" id="oApplyNameInput" />
                </td>
            </tr>
            <tr>
                <th>참가세션 (중복체크가능)</th>
                <td colspan="3" class="end">
                    <input type="checkbox" class="inbox" name="chk_apply_session" id="oApplySesson1" value="1" style="vertical-align:middle;"/> <label for="oApplySesson1" style="vertical-align:middle;">세션1</label>&nbsp;&nbsp;&nbsp;&nbsp;
                    <input type="checkbox" class="inbox" name="chk_apply_session" id="oApplySesson2" value="2" style="vertical-align:middle;"/> <label for="oApplySesson2" style="vertical-align:middle;">세션2</label>&nbsp;&nbsp;&nbsp;&nbsp;
                    <input type="checkbox" class="inbox" name="chk_apply_session" id="oApplySesson3" value="3" style="vertical-align:middle;"/> <label for="oApplySesson3" style="vertical-align:middle;">세션3</label>
                </td>
            </tr>
            <tr>
                <th>소속 및 직함</th>
                <td colspan="3" class="end">
                    <input type="text" name="p_apply_belong_title_input" id="oApplyBelongTitleInput" class="inbox" style="width:200px;" value=""/>
                </td>
            </tr>
            </table>
...
%>
            ...
            <tr>
                ...
                <th>졸업구분</th>
                <td class="end" width="200px"><kocca_select:select sqlNum="0001" name="p_status" styleClass="mg_l10" param="0094"    selectedValue='<%= dbox.get("d_status") %>' isLoad="true" type="2.2"/></td>
            </tr>
            <tr>
                <th>재학기간</th>
                <td colspan="3" class="end">
...
<%} %>
    ...
    <div id="pop_footer">
        <p class="f_btn"><a href="javascript:whenPropose();" class="btn_red"><span>신청하기</span></a><a href="javascript:whenCancel();" class="btn_gr"><span>취소</span></a></p>
    </div>
</form>
</body>
</html>
