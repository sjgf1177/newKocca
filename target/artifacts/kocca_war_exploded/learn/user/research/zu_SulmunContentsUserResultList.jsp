<%
/**
 * file name : zu_SulmunContentsUserPaper_I.jsp
 * date      : 2003/09/12
 * programmer:
 * function  : 설문 응시
 */
...
<body <%=v_onload%>>
<form name="form2" method="post" action="/servlet/controller.research.SulmunRegiostUserServlet">
...
<form name="form3" method="post" action="/servlet/controller.research.SulmunRegistUserServlet">
<!----------------- 타이틀 시작 ----------------->
...
%>
        <tr>
		  ...
		  <%if (dbox.getString("d_sultype").equals(SulmunRegistBean.MULTI_QUESTION)){%>(<%=dbox.getString("d_selmax")%>개까지 선택 가능)<%}%>
		  </td>
        </tr>
<%      if (dbox.getString("d_sultype").equals(SulmunRegistBean.OBJECT_QUESTION)) {   // 단일선택 
...
<%              }
        ...
        } else if (dbox.getString("d_sultype").equals(SulmunRegistBean.MULTI_QUESTION)) {  // 복수선택
            String iscorrect = st.nextToken();
            for (int j=0; j < list.size(); j++) {
                dbox  = (DataBox)list.get(j); 
                if (dbox != null) {
...
<%              } 
        ...
        } else if (dbox.getString("d_sultype").equals(SulmunRegistBean.SUBJECT_QUESTION)) {  // 서술형  
            String iscorrect = st.nextToken();
%>
        <tr> 
...
<%
		} else if (dbox.getString("d_sultype").equals(SulmunRegistBean.COMPLEX_QUESTION)) {  // 복합형
            String iscorrect = st.nextToken();
			StringTokenizer st2 = new StringTokenizer(iscorrect, ":");
...
<%				
        ...
        } else if (dbox.getString("d_sultype").equals(SulmunRegistBean.FSCALE_QUESTION)) {  // 5점척도
            String iscorrect = st.nextToken();
            for (int j=0; j < list.size(); j++) {
                dbox  = (DataBox)list.get(j); 
                if (dbox != null) { 
...
<%          
        ...
        } else if (dbox.getString("d_sultype").equals(SulmunRegistBean.SSCALE_QUESTION)) {  // 7점척도
            String iscorrect = st.nextToken();
            for (int j=0; j < list.size(); j++) {
                dbox  = (DataBox)list.get(j); 
                if (dbox != null) { 
