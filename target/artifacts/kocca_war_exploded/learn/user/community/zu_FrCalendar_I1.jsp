...
<%
    ...
    if(list.size()>0){
       ...
       v_secruity_fg=dbox.getString("d_security_fg");
                 ...
                 <td align="center" valign="top"> 
                  ...
                  <table width="652" border="2" cellspacing="0"   cellpadding="3"   style="border-collapse:collapse;" bordercolor="#ededed" frame="hsides">
                   ...
                   <tr> 
                    ...
                    <td class="tbl_gleft"><input type="radio" name="p_security_fg" value="Y" <%if("Y".equals(v_secruity_fg))out.print(" checked ");%>>공개 
                                          <input type="radio" name="p_security_fg" value="N" <%if("N".equals(v_secruity_fg))out.print(" checked ");%>>비공개
                    </td>
                   </tr>
                   <tr> 
                    ...
                    <td class="tbl_gleft"><select name="p_calday_year">
                     ...
                     <% for(int i=(Integer.parseInt(v_thisday.substring(0,4)));i<=(Integer.parseInt(v_thisday.substring(0,4))+10);i++){%>
                           <option value="<%=i%>" <%if(i==Integer.parseInt(v_calday.substring(0,4)))out.print(" selected");%>><%=i%></option>
                     <%}%>
                     </select>년 
                    <select name="p_calday_month">
                     <option value="00">선택</option>  
                     <% for(int i=1;i<=12;i++){%><option value="<%=i%>" <%if(i==Integer.parseInt(v_calday.substring(4,6)))out.print(" selected");%>><%=i%></option><%}%>
                        </select>월 
                     <select name="p_calday_day" >
                     <option value="00">선택</option>  
                     <% for(int i=1;i<=31;i++){%><option value="<%=i%>"  <%if(i==Integer.parseInt(v_calday.substring(6)))out.print(" selected");%>><%=i%></option><%}%>
                        </select>일 
                     </td>
                    </tr>
                    <tr> 
                     <td width="95" class="<%=v_title_color[1]%>">제목</td>
                     <td class="tbl_btit"><input name="p_title" type="text" value="<%=v_title%>" class="input2" size="60"></td>
                    </tr>
...
%>
                    <tr> 
                     ...
                     <td class="tbl_gleft"><select name="p_ftime_hh">
                      ...
                      <% for(int i=0;i<=11;i++){%><option  value="<%=v_time_cd[i]%>" <%if(v_time_cd[i].equals(v_ftime.substring(0,2)))out.print(" selected");%>>오전<%=v_time_nm[i]%></option><%}%>
                      <% for(int i=12;i<=23;i++){%><option value="<%=v_time_cd[i]%>" <%if(v_time_cd[i].equals(v_ftime.substring(0,2)))out.print(" selected");%>>오후<%=v_time_nm[i]%></option><%}%>
                      </select>시
                      <select name="p_ftime_mm">
                      <option value="xx">선택</option> 
                       <% for(int i=0;i<=59;i++){%><option value="<%=i%>" <%if(!"xx".equals(v_ftime.substring(2))&&i==Integer.parseInt(v_ftime.substring(2)))out.print(" selected");%>><%=i%></option><%}%>
                      </select>분 ~ 
                      <select name="p_ttime_hh">
                      <option value="xx">선택</option> 
                      <% for(int i=0;i<=11;i++){%><option  value="<%=v_time_cd[i]%>" <%if(v_time_cd[i].equals(v_ttime.substring(0,2)))out.print(" selected");%>>오전<%=v_time_nm[i]%></option><%}%>
                      <% for(int i=12;i<=23;i++){%><option value="<%=v_time_cd[i]%>" <%if(v_time_cd[i].equals(v_ttime.substring(0,2)))out.print(" selected");%>>오후<%=v_time_nm[i]%></option><%}%>
                      </select>시
                      <select name="p_ttime_mm">
                      <option value="xx">선택</option> 
                       <% for(int i=0;i<=59;i++){%><option value="<%=i%>" <%if(!"xx".equals(v_ftime.substring(2))&&i==Integer.parseInt(v_ttime.substring(2)))out.print(" selected");%>><%=i%></option><%}%>
                      </select>분까지
                        <input type="checkbox" name="p_day_fg" value="Y"  <%if("Y".equals(v_day_fg))out.print(" checked ");%>>하루종일</td>
                    </tr>
                    <tr> 
                      <td class="<%=v_title_color[1]%>" >장소</td>
                      <td class="tbl_gleft" ><input name="p_cal_area" type="text"  value="<%=v_cal_area%>" class="input2" size="60"></td>
                    </tr>
                    <tr> 
                      <td class="<%=v_title_color[2]%>" >상세내용</td>
                      <td class="tbl_gleft" > <textarea name="p_content" cols="70" rows="3"><%=v_content%> </textarea></td>
                    </tr>
                    <tr> 
                      <td class="<%=v_title_color[1]%>" >반복주기</td>
                      <td class="tbl_gleft" ><select name="p_cycle_time"> 
                      <option value=0>반복안함</option>
                     <% for(int i=1;i<=12;i++){%><option value="<%=i%>" <%if(i==Integer.parseInt(v_cycle_time))out.print(" selected");%>><%=i%></option><%}%>
                        </select>년 
                        <select name="p_cycle_cnt" >
                      <option value="00">선택</option> 
                     <% for(int i=1;i<=12;i++){%><option value="<%=i%>"  <%if(i==Integer.parseInt(v_cycle_cnt))out.print(" selected");%>><%=i%></option><%}%>
                        </select>마다 </td>
                    </tr>
                    <tr> 
                     ...
                     <% for(int i=(Integer.parseInt(v_thisday.substring(0,4))-10);i<=(Integer.parseInt(v_thisday.substring(0,4))+10);i++){%>
                           <option value="<%=i%>"  <%if(i==Integer.parseInt(v_limit_day.substring(0,4)))out.print(" selected");%>><%=i%></option>
                     <%}%>
                      ...
                      <option value="00">선택</option> 
                         <% for(int i=1;i<=12;i++){%><option value="<%=i%>"  <%if(i==Integer.parseInt(v_limit_day.substring(4,6)))out.print(" selected");%>><%=i%></option><%}%>
                        </select>월
                        <select name="p_limit_day_day" >
                      <option value="00">선택</option> 
                         <% for(int i=1;i<=31;i++){%><option value="<%=i%>"   <%if(i==Integer.parseInt(v_limit_day.substring(6)))out.print(" selected");%>><%=i%></option><%}%>
                        </select>일 까지 <strong>또는</strong> 
                        <input type="checkbox" name="p_loop_fg" value="Y"  <%if("Y".equals(v_loop_fg))out.print(" checked ");%>>무한반복</td>
                    </tr>
                  </table>
<!-- 버튼 -->
                  <table width="652" border="0" cellspacing="0" cellpadding="0">
                    ...
                    <tr> 
                      <td align="right"><a href="javascript:uf_listOK();"><img src="../../images/user/button/btn_list.gif"></a><a href="javascript:uf_updateOK();"><img src="../../images/user/button/btn_save.gif" border="0"></a></td>
                    </tr>
                  </table>
                 </td>
                </tr>
