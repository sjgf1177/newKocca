...
function uf_sendOK() {
   ...
   if(rowcnt>0){
    ...
    opener.document.form1.p_limit_list.value=userid;
//    opener.document.form1.p_limit_list_nm.value=kor_name;
    this.close();
}
//전체선택체크
...
function goPage(pageNum) {
    document.form1.p_pageno.value = pageNum;
    document.form1.action = "/servlet/controller.community.CommunityPopUpServlet";
    document.form1.p_process.value = "menumemPage";
    document.form1.submit();
}
...
//상위화면으로 값이동
function uf_sendOK(userid,kor_name,email) {
...
<input type = "hidden" name = "p_cbxflag"      value = "<%=v_cbxflag%>">
    ...
    <div id="pop_container">
        <div id="contentwrap" class="message_top">
			...
			<input type=hidden name=p_list1_kor_name value="<%=v_name%>">
			<input type=hidden name=p_list1_email value="<%=dbox.getString("d_email")%>">
			<tr>
                <td class="end"><input type="checkbox" class="va_up1" name="p_list_userid" value="<%=dbox.getString("d_userid")%>"/></td>
