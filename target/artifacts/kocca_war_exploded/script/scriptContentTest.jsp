		...
		for(int i = 0;  i < list.size(); i ++) {
			...
			if (type.equals("1")&&!"YES".equals(isCheckBox)) {
				...
				scriptSrc.append("<tr class='"+objName+"Class'><td>"+_temp.get("d_codenm")+"("+_temp.get("d_code")
						...
						+		"$($('tr."+objName+"Class').get($($('tr."+objName+"Class').index($(this).parent().parent())-1))).appendTo($($(this).parent().parent()));};\">위</button>"
						+"</td><td><button onclick=\"if($('tr."+objName+"Class').index($(this).parent().parent())<$('#"+objName+"Cnt').val()){"
						+		"$($(this).parent().parent()).appendTo($('tr."+objName+"Class').get($($('tr."+objName+"Class').index($(this).parent().parent())+1)));};\">아래</button>"
						+"</td><td><button id="+objName+"Delete onclick=\"$('#"+objName+"Cnt').val($('#"+objName+"Cnt').val()-1);$(this).parent().parent().remove();\">삭제</button><input type='hidden' name='"+objName+"' value='"+_temp.get("d_code")+"'></td></tr>");
			}
		}
	}else {
