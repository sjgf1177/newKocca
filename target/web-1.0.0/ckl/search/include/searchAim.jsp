...
<h5 class="sch_h5">AIM(검색결과 <%=formatter.format(Long.parseLong(Integer.toString(aim_TotalSize))) %>건) <span class="sch_more"> <a href="javascript:goCategory('aim');" > <img src="<%=ImgUrl%>/ckl/images/common/sch_btn_more.gif" alt="AIM 더보기"> </a> </span> </h5>
	<%}else{%>

<div class="sch_h5">
<h5>AIM(검색결과 <%=formatter.format(Long.parseLong(Integer.toString(aim_TotalSize))) %>건)	</h5>
	<div class="sch_more">
		<label for="seq_Opt_0">
...
<ul class="list_style list_style4">
	<%
	for (int i=0; i < aim_RealSize; i++) {
		for(int k=0 ; k < aim_result.getNumField() ; k++){

			selectFieldTITLE = new String(kistep_getselectSet[k].getField()); //선택할 필드
			if(selectFieldTITLE.equals("NTT_CLS")){
				NTT_CLS = new String(aim_result.getResult(i,k));
			}else if(selectFieldTITLE.equals("NTT_SJ")){
				NTT_SJ = new String(aim_result.getResult(i,k));
			}else if(selectFieldTITLE.equals("NTT_CN")){
				NTT_CN = new String(aim_result.getResult(i,k));
			}else if(selectFieldTITLE.equals("NTT_ORGIN")){
				NTT_ORGIN = new String(aim_result.getResult(i,k));	
			}else if(selectFieldTITLE.equals("NTT_URL")){
				NTT_URL = new String(aim_result.getResult(i,k));
			}else if(selectFieldTITLE.equals("REG_DATE")){
				REG_DATE = new String(aim_result.getResult(i,k));
			}
		}	
...
<!-- 검색의 결과 값이 없을 경우 -->
<h5 class="sch_h5 sch_none"> AIM(검색결과가 존재하지 않습니다) </h5>
<!-- //검색의 결과 값이 없을 경우 -->
<%}%>