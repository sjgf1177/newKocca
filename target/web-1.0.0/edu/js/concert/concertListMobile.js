...
var concertList = {
    ...
    ,registEvent : function(){
        ...
        // 월별 화면의 년도 선택시
        $("#filterMonthlyMonthSelect").on("change", function(){
        ...
        var _isShow = false;
        $(".view_by_date_calendar_text").on("click", function(){
        	_isShow = !_isShow;
        	if( _isShow ){
        		$(".calendar_style_1_wrap").show();
        	}else{
        		$(".calendar_style_1_wrap").hide();
        	}
        })
    ...
    ,setDate : function( isInit ){
        ...
        if( this.listType == 1 ){
            ...
            $(".view_by_date_calendar_text").val( this.calendar.selectedDate );
            ...
            $("#filterMonthlyMonthSelect option[value='"+this.calendar.selectedMonth+"']").prop("selected", "true");
        ...
        if( !isInit ){ // 초기화가 아닐때만 페이지 이동을 처리함.
        	...
        	location.href = "/"+this.siteId+"/concert/info/list.do?menuNo="+ this.menuNo + "&pageIndex="+ 1 + "&siteId="+ this.siteId + "&listType=" + this.listType + "&selectedYear=" + year + "&selectedMonth=" + month + "&selectedDate=" + date;
        }
