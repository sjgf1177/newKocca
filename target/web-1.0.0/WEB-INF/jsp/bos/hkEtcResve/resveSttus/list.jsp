...
$(function(){
	...
	$("#excelDownBtn").click(function() {
		...
		if (confirm("엑셀을 다운로드 하시겠습니까?")) {
			$("#resveSttusForm").attr("action","/bos/hkEtcResve/resveSttus/downloadExcel.xls");
			$("#resveSttusForm").submit();
			$("#resveSttusForm").attr("action","/bos/hkEtcResve/resveSttus/list.do");
		}
		return false;
	...
	$("#useAt_chg_exec").click(function() {
		...
		var url = "/bos/hkEtcResve/resveSttus/chgResveSn.json";
		var params = {
...
function fnSpceNmList(spceNm) {
	var url = "/bos/hkEtcResve/resveSttus/listFcltySpceNm.json";
	var params = {
		areaSe : $("#areaSe").val(),
		lcSe : $("#lcSe").val(),
		fcltySe : $("#fcltySe").val()
...
<form id="resveSttusForm" name="resveSttusForm" method="get" action="/bos/hkEtcResve/resveSttus/list.do">
	...
	<table class="table table-bordered table-striped table-hover">
		...
		<c:forEach var="result" items="${resultList}" varStatus="status">
			<tr>
				...
				<td><a href="/bos/hkEtcResve/resveSttus/view.do?resveSn=<c:out value="${result.resveSn }" />&amp;${pageQueryString}"><c:out value="${result.eqpmnNms }" /></a></td>
...
<!--
<a class="btn btn-primary" href="/bos/hkEtcResve/resveSttus/resveStep1.do?menuNo=<c:out value="${param.menuNo }" />"><span>예약하기</span></a>
 -->
 </div>
