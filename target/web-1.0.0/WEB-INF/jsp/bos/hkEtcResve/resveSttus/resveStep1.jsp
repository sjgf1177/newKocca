...
<c:if test="${empty result}">
	<c:set var="action" value="/bos/hkEtcResve/fclty/insert.do" />
	<c:set var="actTp" value="insert" />
</c:if>
<c:if test="${not empty result}">
	<c:set var="action" value="/bos/hkEtcResve/fclty/update.do" />
...
$(function() {
	...
	$("#prevBtn").click(function() {
		$(".reservSysSet").fadeOut("slow");
		return false;
		//window.location.href = "/<c:out value="${paramVO.siteName}" />/hkEtcResve/resveSttus/list.do?menuNo=<c:out value="${param.menuNo}" />";
		//return false;
	});
		...
		if (data) {
			...
			$.each(data.resultList, function(key,item) {
				...
				var $btnItem = $("<a>").attr({
					...
					'href': "/ckl/hkEtcResve/fclty/viewPopup.do?fcltySn="+item.fcltySn + "&viewType=BODY",
...
function fnResveSave(resveTp) {
	...
	var url = "/bos/hkEtcResve/resveSttus/resveStep2.do";
