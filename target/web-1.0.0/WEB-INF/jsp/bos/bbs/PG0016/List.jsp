...
<%-- 							<td class="output"><input type="checkbox" name="nttIdData" id="nttIdData_${result.nttId}" class="input_check" value="<c:out value="${result.nttId}"/>" /></td> --%>
							...
							<td class="tit"><c:url var="url" value="/bos/bbs/${paramVO.bbsId}/view.do?nttId=${result.nttId}&${pageQueryString}" /> <a href="${url}">${result.nttSj}</span>
							</a></td>

							<td class="output">
								<c:forEach var="code" items="${psitnCodeList }">
									<c:if test="${result.option1 eq code.insttSn }">
										<c:out value="${code.insttNm}" />
									</c:if>
								</c:forEach>
							</td>

							<td class="output"><c:if test="${not empty result.atchFileId and result.fileCount > 0}">
									<a href="/bos/cmm/fms/FileDown.do?atchFileId=${result.atchFileId}&amp;fileSn=${result.fileSn}"> <img src="/bos/images/common/disk.gif" alt="${result.orignlFileNm}" />
									</a>
								</c:if></td>
