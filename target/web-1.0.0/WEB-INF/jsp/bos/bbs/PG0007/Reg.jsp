	...
	<div class="bdView">
		<table summary="<c:if test="${roleSuper eq 'Y'}">게시구분,게시기간,</c:if>제목,담당부서,내용<c:if test="${not empty result}">등록일,</c:if><c:if test="${not empty result}">조회수,</c:if> 포함">
			...
			<tbody>
				<!-- 이름 임시 -->
				<tr>
					<th scope="row"><label for="option1">성명</label></th>
					<td>
						<input type="text" name="option1" id="option1" class="w500" value="${result.option1}" />
					</td>
				</tr>
				...
				<tr>
					<th scope="row"><label for="option3">소속분야</label></th>
					<td>
							<select name="option3" title="소속분야">
							<option value="">분야선택</option>
							<c:forEach var="code" items="${COM091CodeList}" varStatus="status" >
					...
					<tr>
					<th scope="row"><label for="option4">소속기관</label></th>
					<td>
						<input type="text" name="option4" title="소속기관" id="option4" class="w500" value="${result.option4}" />
					</td>
				</tr>
