...
<div class="bdView">
		...
		<table summary="<c:if test="${not empty result}">등록일,</c:if><c:if test="${not empty result}">조회수,</c:if> 포함">
			...
			<tbody>
				...
				<tr>
					...
					<td>
						<a href="<c:if test="${fn:indexOf(result.option1,'http') eq -1 }">https://</c:if>
							<c:out value="${result.option1}" />" target="_blank" title="새창열림"><c:out value="${result.option1}"  escapeXml="false"/>
						</a>
					</td>
				</tr>
