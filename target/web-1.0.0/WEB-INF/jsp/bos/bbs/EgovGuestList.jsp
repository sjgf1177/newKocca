...
<script type="text/javascript">
	function fn_egov_select_guestList(pageNo) {
		...
		document.board.action = "<c:url value='/admin/bbs/selectGuestList.do'/>";
		document.board.submit();
	}

	function fn_egov_selectForupdt(nttId) {
		document.board.nttId.value = nttId;
		document.board.action = "<c:url value='/admin/bbs/selectSingleGuestList.do'/>";
		document.board.submit();
	}
	...
	function fn_egov_deleteGuestList(nttId) {
		if (confirm('<spring:message code="common.delete.msg" />')) {
			document.board.nttId.value = nttId;
			document.board.action = "<c:url value='/admin/bbs/deleteGuestList.do'/>";
			document.board.submit();
		}
	}
	...
	function fn_egov_updt_guestList() {
		...
		if (confirm('<spring:message code="common.update.msg" />')) {
			document.board.action = "<c:url value='/admin/bbs/updateGuestList.do'/>";
			document.board.submit();
		}
	}
	...
	function fn_egov_insert_guestList() {
		...
		if (confirm('<spring:message code="common.regist.msg" />')) {
			document.board.action = "<c:url value='/admin/bbs/insertGuestList.do'/>";
			document.board.submit();
		}
	}
</script>
...
<div id="border" style="width:730px">
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="1" class="table-register">
	  <tr>
	    ...
	    <td width="30%" nowrap>
	      <input name="ntcrNm" type="text" size="20" value='<c:out value="${boardVO.ntcrNm}" />' maxlength="20">
	    </td>
	  </tr>
	  <tr>
	    ...
	    <td>
	      <textarea name="nttCn" class="textarea"  cols="50" rows="4"  style="width:100%;"><c:out value="${boardVO.nttCn}" /></textarea>
