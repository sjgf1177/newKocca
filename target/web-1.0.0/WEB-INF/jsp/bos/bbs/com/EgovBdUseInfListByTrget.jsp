...
<script type="text/javascript">
	...
	function fn_egov_select_bbsUseInfs(pageNo) {
		...
		document.frm.action = "<c:url value='/cop/com/selectBBSUseInfsByTrget.do'/>";
		document.frm.submit();
	}
	 ...
	 <c:forEach var="result" items="${resultList}" varStatus="status">
    	...
    	<c:choose>
    		<c:when test="${result.useAt=='Y'}">
			    ...
			    <td class="lt_text3" nowrap>
			    	<input type="button" class="btn btn-default" value="사용중지" onClick="javascript:fn_egov_delete_bbsUseInf('<c:out value="${result.bbsId}"/>','<c:out value="${result.trgetId}"/>');" />
			    </td>
    		</c:when>
    		<c:when test="${result.useAt=='N'}">
			    ...
			    <td class="lt_text3" nowrap>
			    	<input type="button" class="btn btn-default" value="사용" onClick="javascript:fn_egov_update_bbsUseInf('<c:out value="${result.bbsId}"/>','<c:out value="${result.trgetId}"/>');" />
			    </td>
    		</c:when>
    		<c:otherwise>
		 	    <td class="lt_text3" nowrap>
			    ...
			    <td class="lt_text3" nowrap>
			    	<input type="button" class="btn btn-default" value="사용" onClick="javascript:fn_egov_insert_bbsUseInf('<c:out value="${result.bbsId}"/>','<c:out value="${result.trgetId}"/>');" />
			    </td>
    		</c:otherwise>
    	</c:choose>
