		...
		<div class="bdView">
			<table summary="<c:if test="${roleSuper eq 'Y'}">게시구분,게시기간,</c:if>제목,담당부서,내용<c:if test="${not empty result}">등록일,</c:if><c:if test="${not empty result}">조회수,</c:if> 포함">
				...
				<tbody>
					...
					<tr>
						<th scope="row"><label for="option1">상단 고정 여부</label></th>
						<td>
							<label>
								<input type="radio" name="option1" id="option1" value="Y" ${result.option1 eq 'Y' || result.parent eq null?'checked':''} />
								사용
							</label>
							<label>
								<input type="radio" name="option1" id="option1" value="N" ${result.option1 eq 'N'?'checked':''} />
								미사용
							</label>
						</td>
					</tr>
					<tr>
						<th scope="row"><label for="option2">배경색</label></th>
						<td><input type="text" name="option2" id="option2" maxlength="7" value="<c:out value="${result.option2}" />" /></td>
					</tr>		
					<tr>
					...
					<c:if test="${masterVO.fileAtchPosblAt eq 'Y'}">
						<c:if test="${fn:length(fileList) gt 0}">
							<c:forEach var="x" begin="0" end="${fn:length(fileList)-1}">
								<c:if test="${fileList[x].fileFieldName eq 'sub_file' or fileList[x].fileCn eq 'sub_file'}">
									<c:set var="fileVO" value="${fileList[x]}"/>
								</c:if>
							</c:forEach>
						</c:if>
						<tr>
							...
							<td>
								<c:if test="${not empty fileVO}">
									...
									<a href="#" onclick="javascript:delFile2('${fileVO.atchFileId}', '${fileVO.fileSn}', '${masterVO.bbsId}','sub_file');">
										<img src="<c:url value='/bos/images/btn_X.jpg'/>" alt="<c:out value="${fileVO.fileCn}"/> 삭제" />
									</a>
								</c:if>
								<c:if test="${empty fileVO}">
									<input name=sub_file type="file" id="sub_file" class="input_file form-control" title="첨부파일" />
								</c:if>
								<c:set var="fileVO" value=""/>
							</td>
						</tr>
