...
$(function() {
	...
	$("#schdulBtn").click(function() {
		if(confirm("일정으로 등록하시겠습니까?")) {
			var url = "/bos/hkFcltyResve/schdul/insert.json";
			var params = {
				eventDt : $("#resveDt").val(),
	...
	$("#cmmntSaveBtn").click(function() {
		...
		var url = "/bos/hkFcltyResve/resveSttus/saveCmmnt.json";
		var param = {
...
function fnConfirmProcess(resveSttus) {
	var url = "/bos/hkFcltyResve/resveSttus/updateResveSttus.json";
	params = {
...
<c:if test="${fn:length(listEqpmnByResve) > 0 }">
<h4>장비 신청정보</h4>
<div class="bdList">
	<table class="table table-striped" summary=" ">
		...
		<tbody>
		<c:forEach var="eqpmnItem" items="${eqpmnList }" varStatus="status">
			<tr>
				<td><c:out value="${eqpmnItem.eqpmnNm }" /></td>
...
<div class="row">
	...
	<c:if test="${result.resveSttus eq '03' }">
		<a href="/bos/hkFcltyResve/resveSttus/resveStep1.do?resveSn=<c:out value="${result.resveSn }" />&menuNo=<c:out value="${param.menuNo }" />" id="waitUpdBtn" class="btn btn-danger">대기예약정보변경</a>
	</c:if>
	</div>
	...
	<div class="col-md-2"><a class="btn btn-primary" href="/bos/hkFcltyResve/resveSttus/list.do?${pageQueryString }"><span>목록</span></a></div>
</div>
