...
function fncAddDeptAuthorInsert() {
    ...
    if(confirm("복원하시겠습니까?")) {
        document.listForm.action = "<c:url value='/bos/member/checkRestore.do'/>";
...
<form method="post" name="listForm" action="<c:url value='/bos/member/checkList.do'/>">
	<div id="content">
		<div class="hgroup">
			<h3>로그인 복원(관리자)</h3>
		</div>
	...
	<div>
		<table class="table table-bordered table-striped table-hover">
			...
			<c:forEach var="x" begin="0" end="${fn:length(resultList)-1}">
				<tr>
					<td><input type="checkbox" name="checkId" value="<c:out value="${resultList[x].aid}"/>"/> </td>
					<td>${resultList[x].aid}</td>
					<td>${resultList[x].adnm}</td>
					<td>${resultList[x].larNm}</td>
					<td>${resultList[x].midNm}</td>
					<td>${resultList[x].smNm}</td>
				</tr>
