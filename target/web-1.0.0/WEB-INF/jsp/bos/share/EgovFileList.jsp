		...
		<li>
			...
			<c:choose>
				<c:when test="${updateFlag=='Y' or not empty result.nttId}">
					...
					<c:if test="${fileVO.fileFieldName ne 'thumbImage'}">
					...
					<c:if test="${(paramVO.siteName ne 'portal' and fileVO.fileFieldName ne 'thumbImage') or param.updateFlag=='Y'}">
						<c:if test="${!empty param.bbsId || param.bbsId != ''}">
			...
			<c:if test="${paramVO.bbsId eq 'B0000051'}">[/bos/cmm/fms/FileDown.do?atchFileId=${fileVO.atchFileId}&fileSn=${fileVO.fileSn}]</c:if>
		</li>
	</c:forEach>
</ul>
</c:if>
