...
<div class="mt30 mb30 tac">
	...
	<!-- search Panel -->
	<div class="schPanel">
		<form action="/edumobile/job/empmnInfo/list.do" name="empmnForm" id="empmnForm" method="get">
			...
			<fieldset>
				...
				<div class="mb20">
					<table class="contents_insight_view contact_us_view work_tb">
						<caption>(콘텐츠아카데미) 채용정보 목록</caption>
						<colgroup>
							<col style="width:20%;" />
							<col />
						</colgroup>
	...
	<div class="col-12 sub_board_body">
		<table class="board_type_0 small" style="word-break:break-all;">
			<caption>(콘텐츠아카데미) 직업 사전 목록</caption>
			<colgroup>
			<col style="width:13%;" class="web_table_column_17"/>
			<col  />
			<col style="width:13%;" class="web_table_column_20"/>
			<col style="width:10%;" class="web_table_column"/>
			<col style="width:13%;" class="web_table_column_17"/> 
			</colgroup>
			<thead>
				<tr>
					...
					<th scope="col" class="web_table_cell">등록일</th>
					<th scope="col">마감일</th>
				</tr>
			</thead>
			<tbody>
			<c:forEach var="result" items="${list}" varStatus="status">
				<tr>
					...
					<td class="tal"><a href="/edumobile/job/empmnInfo/view.do?wantedAuthNo=${result.wantedAuthNo }&${pageQueryString}"><c:out value="${result.title }" /> / <c:out value="${result.holidayTpNm }" />  / <c:out value="${result.region }" /></a></td>
					<td>
						<span class="db"><c:out value="${result.minEdubg }" /></span>
						<span class="db"><c:out value="${result.career }" /></span>
					</td>
					<td class="web_table_cell"><c:out value="${result.regDt }" /></td>
