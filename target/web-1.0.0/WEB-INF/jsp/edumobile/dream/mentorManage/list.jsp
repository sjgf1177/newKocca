...
//<![CDATA[
	function mentoSubmit() {
		$urlData = $("#insttSnData").val();
...
<div class="tab_style_1_con">
	<ul class="tab_style_1 three_tab size_33">
		...
		<li class="active"><a href="/edumobile/dream/mentorManage/list.do?menuNo=600106" title="선택됨"><span>멘토라인업</span></a></li>
	</ul>
</div>
	...
	<div class="col-5 text_box">
		...
		<c:forEach var="code" items="${psitnCodeList }" varStatus="status">
			<li <c:if test="${curInsttSn eq  code.insttSn }">class="active"</c:if>>
				<a <c:if test="${curInsttSn eq  code.insttSn }">title="선택됨"</c:if> href="/edumobile/dream/mentorManage/list.do?menuNo=<c:out value="${paramVO.menuNo }"/>&amp;insttSn=<c:out value="${code.insttSn}"/>">
...
<div class="mobile_full_select_box_con">
	<div class="col-12 pb10">
		<label for="mentorlineup_list" title="플랫폼 기관 선택"><플랫폼 기관></label>
	</div>
	<select name="mentorlineup_list" id="mentorlineup_list">
		<c:set var="curInsttSn" value="${param.insttSn}"/>
		<c:forEach var="code" items="${psitnCodeList }" varStatus="status">
			<option value="/edumobile/dream/mentorManage/list.do?menuNo=<c:out value="${paramVO.menuNo }"/>&amp;insttSn=<c:out value="${code.insttSn}"/>">
...
<script type="text/javascript">
    ...
    $(function(){
    	var optionList = $("#mentorlineup_list").find(">option");
    	var url = window.location.href;
    	var cnt = 0;
    	console.log( url )
    	for( var i=0; i<optionList.length; i++){
    		var optionVal = $(optionList[i]).attr('value');
    		if( url.indexOf(optionVal) > -1 ){
    			$(optionList[i]).attr('selected', 'selected');
    			cnt++;
    			break;
    		}
    	}
    	if( cnt >= optionList.length ) $(optionList[0]).attr('selected', 'selected');
...
<%-- <div class="alert formLine1 formLine">
		<form action="/edumobile/dream/mentorManage/list.do" method="post" class="form-inline" onsubmit="return mentoSubmit();" >
			<input type="hidden" name="menuNo" value="${param.menuNo }">
			<input type="hidden" name="pageQueryString" value="searchCnd=&amp;amp;searchWrd=&amp;amp;gubun=&amp;amp;delCode=0&amp;amp;useAt=&amp;amp;replyAt=&amp;amp;menuNo=500037&amp;amp;sdate=&amp;amp;edate=&amp;amp;viewType=&amp;amp;type=&amp;amp;siteId=&amp;amp;option1=&amp;amp;option5=&amp;amp;pageIndex=1">
			<fieldset>
				<legend>검색 입력폼</legend>
				<div>
					<label for="searchCnd" class="tac hidden"><strong>구분선택</strong></label>
					<select name="insttSnData" id="insttSnData" title="검색 구분을 선택해 주세요."  onchange="window.location.href=this.value;"  >
						<c:forEach var="code" items="${psitnCodeList }" varStatus="status">
							<option value="list.do?menuNo=${param.menuNo }&amp;insttSn=<c:out value="${code.insttSn}" />" <c:if test="${(insttSn eq code.insttSn) or (param.insttSn eq code.insttSn)}">selected="selected"</c:if>><c:out value="${fn:trim(code.insttNm)}" /></option>
						</c:forEach>
					</select>
					<button class="btn btn-sm btn-black" title="구분 선택">선택</button>
				</div>
			</fieldset>
		</form>
	</div>
	<div class="photo4Profile photo4Profile4"><!-- 4n+4의 간격으로 mr0클래스 필수 -->
		<ul>
		<c:forEach var="result" items="${resultList}" varStatus="status">
		<c:choose>
			<c:when test="${status.index%11 eq 0 }">
		...
		<c:set var="files" value="${fileMap[result.atchFileId] }" />
		<c:forEach var="fileVO" items="${files }" >
				<c:if test="${fileVO.fileFieldName eq 'photo'}">
					<c:set var="file" value="${fileVO}" />
				</c:if>
			</c:forEach>
			<li>
				<dl style="background-color:<c:out value='${colorCode }'/>;">
					<dt>
					<c:if test="${not empty file }">
					 <img src="/cmm/fms/getImage.do?atchFileId=${file.atchFileId}&amp;fileSn=${file.fileSn}" alt="<c:out value="${result.nm } 멘토" />" />
					</c:if>
						<span class="fs14 mt15"><c:out value="${result.nm }" /> 멘토</span><br>
						<span class="fwn"><c:out value="${result.psitn }" /></span>
					</dt>
					<dd>
						<dl class="mt10">
							<dt><c:out value ="${result.histSe eq '1' ? '대표작' : '약 력'}" /> :  </dt>
							<dd><c:out value="${result.histCn }" /></dd>
							<dt>멘 티  :</dt>
							<dd><c:out value="${result.mentoNm }" /></dd>
						</dl>
					</dd>
				</dl>
			</li>
