...
<c:if test="${userVO.userIdx eq '0' }">
	<form class="login_form" name="loginForm" id="loginForm_sub" action="#" method="post">
	...
	$(function(){
		...
		if ( confirm("로그인 페이지로 이동합니다.") ){
			...
			$form.attr( "action", "/${paramVO.siteName}/member/forLogin.do?menuNo=600130" );
			$form.submit();
		} else {
			history.back();
		}
			...
			<form action="/${paramVO.siteName}/concert/info/history.do?menuNo=${paramVO.menuNo}" method="post" id="historyFilter">
			    ...
			    <div class="view_by_period_search_con" id="periodSearchCon" style="display:none; <c:if test="${paramVO.searchType ne 'datelySearch'}">display:block;</c:if>">
			        <div class="view_by_period_item_con">
			        	<button class="view_by_period_item <c:if test="${paramVO.periodSelectDate eq 3}">on</c:if>" <c:if test="${paramVO.periodSelectDate eq 3}"> title="선택됨"</c:if> onclick="setPeriodDate(this); return false;" value="3">3일</button>
			        	<button class="view_by_period_item <c:if test="${paramVO.periodSelectDate eq 7}">on</c:if>" <c:if test="${paramVO.periodSelectDate eq 7}"> title="선택됨"</c:if> onclick="setPeriodDate(this); return false;" value="7">7일</button>
			        	<button class="view_by_period_item <c:if test="${paramVO.periodSelectDate eq 15}">on</c:if>" <c:if test="${paramVO.periodSelectDate eq 15}">title="선택됨"</c:if> onclick="setPeriodDate(this); return false;" value="15">15일</button>
			        	<button class="view_by_period_item <c:if test="${paramVO.periodSelectDate eq 30}">on</c:if>" <c:if test="${paramVO.periodSelectDate eq 30}">title="선택됨"</c:if> onclick="setPeriodDate(this); return false;" value="30">1개월</button>
			        	<button class="view_by_period_item <c:if test="${paramVO.periodSelectDate eq 90}">on</c:if>" <c:if test="${paramVO.periodSelectDate eq 90}">title="선택됨"</c:if> onclick="setPeriodDate(this); return false;" value="90">3개월</button>
			        	<button class="view_by_period_item <c:if test="${paramVO.periodSelectDate eq 180}">on</c:if>" <c:if test="${paramVO.periodSelectDate eq 180}">title="선택됨"</c:if> onclick="setPeriodDate(this); return false;" value="180">6개월</button>
			        	<button class="view_by_period_item <c:if test="${paramVO.periodSelectDate eq 365}">on</c:if>" <c:if test="${paramVO.periodSelectDate eq 365}">title="선택됨"</c:if> onclick="setPeriodDate(this); return false;" value="365">1년</button>
			        </div>
			    </div>
					...
					<div class="view_by_period_item_con">
					    <div class="select_style_2_wrap">
					        <div class="select_style_2_con testbed_style">
					            <select name="searchCnd" id="searchCnd" class="select_style_2" title="예매일 관람일 선택">
					            ...
					            <label for="searchCnd" class="back_label" aria-hidden="false">예매일 관람일 선택</label>
					        </div>
					    </div>
					    ...
					    <div class="show-md">
						    <div class="view_by_date_calendar_con">
						    	...
						    	<input type="text" class="view_by_date_calendar_text" id="sdate" name="sdate" value="${param.sdate}" title="시작일 선택" data-datepicker>
						    	<label for="sdate" class="back_label" aria-hidden="false">시작일 선택</label>
							</div>
							...
							<div class="view_by_date_calendar_con">
					            ...
					            <input type="text" class="view_by_date_calendar_text" id="edate" name="edate" value="${param.edate}" title="종료일 선택" data-datepicker>
					            <label for="edate" class="back_label" aria-hidden="false">종료일 선택</label>
					        </div>
				        </div>
