...
function del(){
	var form = $("#board")[0];
	form.action = "/edumobile/bbs/${paramVO.bbsId}/delete.do?menuNo=<c:out value="${param.menuNo }"/>";
	form.submit();
}
</script>
</c:if>
	...
	<table class="board_type_0 detail notice">
		<%-- <caption>(콘텐츠아카데미) 공지사항 상세 보기</caption> --%>
		<caption>창의클럽 커뮤니티 상세 - 제목, 조회수, 작성일 내용정보를 제공하는 표</caption>
		<colgroup>
			<col width="100%">
		</colgroup>
		<thead>
	...
	<c:if test="${idCeck eq result.frstRegisterId  }">
		<div class="board_util_btn_con">
			<a href="/edumobile/bbs/${paramVO.bbsId}/forUpdate.do?nttId=${result.nttId}&${pageQueryString}&${result.delcode}" class="btn_style_0 full list">
				수정
			</a>
