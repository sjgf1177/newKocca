...
<div class="schPanel">
	...
	<input type="hidden" name="pblancSn" value="${empty result.pblancSn ? 0 : result.pblancSn }" />
		<fieldset>
			...
			<div class="bdView mb0">
				...
				<tbody>
					...
					<tr>
						...
						<td><input type="text" name="pblancNm" id="pblancNm" value="<c:out value="${param.pblancNm }" />"></td>
					</tr>
					<tr>
						<th scope="row">학력</th>
						<td>
					...
					<tr>
						...
						<c:forEach var="code" items="${COM100CodeList }" varStatus="status">
							...
							<c:if test="${code.code eq '02' }">
								<input type="text" id="careerMin1" title="경력 시작 월을 입력해 주세요." name="careerMin" style="width:50px;" value="${param.careerMin }"  />~
								<input type="text" id="careerMax1" title="경력 종료 월을 입력해 주세요." name="careerMax" style="width:50px;" value="${param.careerMax }"  />
			...
			<div class="btnSet mb0 tac">
				<input type="submit"  class="btn btn-primary" value="검색" />
				<input type="button"  class="btn btn-primary resetButton"  value="초기화" />
			</div>
		</fieldset>
	</form>
</div>
...
<div class="mb5">
	<div>총 <strong class="fcRed"><c:out value="${resultCnt }" /></strong>건의 검색 결과가 있습니다.</div>
</div>
