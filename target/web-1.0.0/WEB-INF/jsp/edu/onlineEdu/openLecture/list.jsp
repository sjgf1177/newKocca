...
<div class="over-hidden sub_contents_header">
	<div class="linemap_wrap"> <%-- fl class 삭제 --%>
		<ul class="col-12 linemap_con">
			...
			<li><a href="javascript:void(0);" tabindex="-1"><span>바로 수강</span></a></li>
		</ul>
	</div>
</div>
					...
					<div class="fwo_snail_box">
						...
						<div class="fwo_info_box">
							<h3 class="fwo_tit_box">
								<a href='<c:out value="${url }" />&amp;${pageQueryString }'><c:out value='${result.lecnm }' escapeXml="false" /></a>
							</h3>
...
<c:if test="${fn:length(resultList) > 0}">
	...
	<c:forEach items="${resultList }" var="result" varStatus="status">
		...
		<c:if test="${status.first eq false}">
			<c:if test="${mainTitle ne result.g2nm}">0
				</div>
				</div>
