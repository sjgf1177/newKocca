...
.tabs{
 ...
 .reservSet>ul>li.stp01{
 	height: 300px;
 }
 .reservSet>ul>li.stp02{
 	height: 300px;
 }
...
$(function() {
	...
	$("#prevBtn").click(function() {
		window.location.href = "/<c:out value="${paramVO.siteName}" />/hkFcltyResve/resveSttus/step1.do?menuNo=<c:out value="${param.menuNo}" />";
		return false;
	});
	...
	$("#spceNmDiv").delegate(".spceViewBtn","click",function() {
		var params = {viewType : "CONTBODY", fcltySn : $(this).attr("data-value")};
	  	$("#spceViewPopupCont").load("/bos/hkFcltyResve/fclty/view.do", params, function() {
...
function fnSpceNmList(obj, pAreaSe, pLcSe, pFcltySe) {
	...
	var $obj = obj;
	var url = "/<c:out value="${paramVO.siteName}" />/hkFcltyResve/resveSttus/listFcltySpceNm.json";
	var params = {
		areaSe : pAreaSe,
		lcSe : pLcSe,
		fcltySe : pFcltySe
		...
		if (data) {
			...
			$.each(data.resultList, function(key,item) {
				...
				var $btnItem = $("<a>").attr({
					...
					'href': "/edu/hkFcltyResve/fclty/viewPopup.do?fcltySn="+item.fcltySn + "&viewType=BODY",
...
function fnLoadCanlendar(obj, pFcltySn, pYear, pMonth, pDay, chkSum) {
	...
	var $obj = obj;
	$obj.empty();
	var url = "/<c:out value="${paramVO.siteName}" />/hkFcltyResve/resveSttus/listCal.do";
	var params = {
		fcltySn : pFcltySn,
		year  : pYear,
		month : pMonth,
...
function fnLoadTimes(obj,pDay, resveTp) {
	...
	var url = "/<c:out value="${paramVO.siteName}" />/hkFcltyResve/resveSttus/listTime.do";
	var params = {
		resveTp : resveTp,
		resveDt  : resveDt,
		fcltySn : fcltySn,
...
function fnLoadEqpmnList(obj) {
	...
	var url = "/<c:out value="${paramVO.siteName}" />/hkFcltyResve/resveSttus/listEqpmn.do";
	var params = {
		resveDt  : resveDt,
		beginTime : beginTime,
		endTime : endTime,
...
function fnResveSave(resveTp) {
	...
	var url = "/<c:out value="${paramVO.siteName}" />/hkFcltyResve/resveSttus/step3.do";
...
<!-- <div class="well">관련 문의사항 : 02-6310-0627</div> -->
		...
		<div class="btnSet board_util_btn_con">
			<a href="#self" class="btn_style_0 green radius" id="agreeBtn">시설 예약 운영정책에 동의합니다.</a>
		</div>
