...
<tr>
	<th scope="row"><c:if test="${param.addEssntlAt eq 'Y'}"><span class="hide_star"><span class="ally-hidden">(필수입력)</span></span></c:if> <c:out value="${param.iemNm}"/></th>
	<td>
		...
		<c:choose>
			<c:when test="${param.attrb eq '01'}">
				...
				<c:if test="${empty param.mode}">
					<input type="text" name="attrbDataCn${idx}" id="attrbDataCn${idx}" style="width:130px" value='<c:out value="${param.attrbDataCn}"/>' title="<c:out value="${param.iemNm}"/>" />
				</c:if>
			</c:when>
			<c:when test="${param.attrb eq '02'}">
				...
				<c:if test="${empty param.mode}">
					<input type="text" name="attrbDataCn${idx}" id="attrbDataCn${idx}" style="width:230px" value='<c:out value="${param.attrbDataCn}"/>' title="<c:out value="${param.iemNm}"/>" />
				</c:if>
			</c:when>
			<c:when test="${param.attrb eq '03'}">
				...
				<c:if test="${empty param.mode}">
					<input type="text" name="attrbDataCn${idx}" id="attrbDataCn${idx}" style="width:100%; max-width:330px" value='<c:out value="${param.attrbDataCn}"/>' title="<c:out value="${param.iemNm}"/>" />
				</c:if>
			</c:when>
			<c:when test="${param.attrb eq '04'}">
				...
				<c:if test="${empty param.mode}">
					<input type="text" name="attrbDataCn${idx}" id="attrbDataCn${idx}" class="sdate" title="<c:out value="${param.iemNm}"/> 입력" style="width:130px" value='<c:out value="${param.attrbDataCn}"/>' readonly="readonly"/>
				</c:if>
			</c:when>
			...
			<c:when test="${param.attrb eq '06'}">
				...
				<c:if test="${empty param.mode}">
					<select name="attrbDataCn${idx}" id="attrbDataCn${idx}" title="<c:out value="${param.iemNm}"/>">
			...
			<c:when test="${param.attrb eq '07'}">
				<c:if test="${param.mode eq 'M'}">
					...
					<c:forEach var="code" items="${iemCodeList}" varStatus="status">
						<c:if test="${fn:indexOf(param.attrbDataCn, code.code) > -1}">
							<c:if test="${num > 0}">, </c:if>
							${code.codeNm}
							<c:set var="num" value="${num+1}"/>
						</c:if>
					</c:forEach>
					<c:if test="${fn:indexOf(param.attrbDataCn, 'CE') > -1}">
						(기타)<br/>
						${fn:replace(param.attrbDataEtc, newLineChar,"<br/>")}
					</c:if>					
				</c:if>
				<c:if test="${empty param.mode}">
					<c:forEach var="code" items="${iemCodeList}" varStatus="status">
						<input type="checkbox" name="attrbDataCn${idx}" value="${code.code}" data="${idx}" class="input_delete checkbox-value" id="attrbDataCn${idx}_${status.count}" <c:if test="${fn:indexOf(param.attrbDataCn, code.code) > -1}">checked="checked"</c:if>/>
						<label for="attrbDataCn${idx}_${status.count}">${code.codeNm}</label><br/>
					</c:forEach>
					<input type="checkbox" name="attrbDataCn${idx}" value="CE" data="${idx}" class="input_delete checkbox-value" id="attrbDataCn${idx}_CE" <c:if test="${fn:indexOf(param.attrbDataCn, 'CE') > -1}}">checked="checked"</c:if>/>
					<label class="input-interval" for="attrbDataCn${idx}_CE">기타</label>
					<textarea name="attrbCetc${idx}" id="attrbCetc${idx}" cols="120" rows="10" class="col-md-12" disabled></textarea><br/>
				</c:if>
			</c:when>
			<c:when test="${param.attrb eq '08'}">
				<c:if test="${param.mode eq 'M'}">
					...
					<c:if test="${param.attrbDataCn eq 'RE'}">
						(기타)<br/>
						<p>${fn:trim(param.attrbDataEtc)}</p>
					</c:if>
				</c:if>
				<c:if test="${empty param.mode}">
					<c:forEach var="code" items="${iemCodeList}" varStatus="status">
						<input type="radio" name="attrbDataCn${idx}" value="${code.code}" data="${idx}" class="input_delete radio-value" id="attrbDataCn${idx}_${status.count}" <c:if test="${param.attrbDataCn eq code.code}">checked="checked"</c:if>/>
						<label class="input-interval" for="attrbDataCn${idx}_${status.count}">${code.codeNm}</label><br/>
					</c:forEach>
					<input type="radio" name="attrbDataCn${idx}" value="RE" data="${idx}" class="input_delete radio-value" id="attrbDataCn${idx}_RE" <c:if test="${param.attrbDataCn eq 'RE'}">checked="checked"</c:if>/>
					<label class="input-interval" for="attrbDataCn${idx}_RE">기타</label>
					<textarea name="attrbRetc${idx}" id="attrbRetc${idx}" cols="120" rows="10" class="col-md-12" disabled></textarea><br/>
				</c:if>
			</c:when>
			...
			<c:when test="${param.attrb eq '10'}">
				<c:if test="${param.mode eq 'M'}">
					${fn:replace(param.attrbDataCn, newLineChar,"<br/>")}
				</c:if>
				<c:if test="${empty param.mode}">
					<textarea name="attrbDataCn${idx}" id="attrbDataCn${idx}" cols="120" rows="12" class="col-md-12" onfocus="if(this.value=='내용을 입력하여 주세요.')this.value='';" onkeyup="if(this.value=='내용을 입력하여 주세요.')this.value='';" title="<c:out value="${param.iemNm}"/> 입력칸입니다.">${param.attrbDataCn}</textarea>
				</c:if>
			</c:when>
			...
			<c:when test="${param.attrb eq '14'}">
				...
				<c:if test="${empty param.mode}">
					<c:set var="data" value="${fn:split(param.attrbDataCn,'|')}" />
					<textarea name="attrbDataCn${idx}_1" id="attrbDataCn${idx}_1" cols="120" rows="12" class="col-md-12" onfocus="if(this.value=='내용을 입력하여 주세요.')this.value='';" onkeyup="if(this.value=='내용을 입력하여 주세요.')this.value='';"  title="동의 내용을 입력해 주세요."><c:out value="${data[0]}"/></textarea>
					<input type="checkbox" name="attrbDataCn${idx}_2" id="attrbDataCn${idx}_2" value="Y" <c:if test="${data[1] eq 'Y'}">checked="checked"</c:if>/><label for="attrbDataCn${idx}_2" class="mt10 w100p"><c:out value="${param.iemNm}"/>(항목명)에 동의합니다.</label>
				</c:if>
			</c:when>
				...
				<c:if test="${empty param.mode}">
					...
					<div id="file_${idx}">
						<c:if test="${not empty files}">
				    		...
				    		<a href="/edu/cmm/fms/FileDown.do?atchFileId=${files[0].atchFileId}&amp;fileSn=${files[0].fileSn}">
						...
						<c:if test="${empty files}">
							<input type="file" name="attrbDataCn${idx}" id="attrbDataCn${idx}" title="<c:out value="${param.iemNm}"/> 등록" class="col-12 col-md-6" style="border:1px solid #e3e6e9; border-radius:3px;"/>
							<c:if test="${fn:indexOf(param.iemNm, '이력서') > -1 }">
