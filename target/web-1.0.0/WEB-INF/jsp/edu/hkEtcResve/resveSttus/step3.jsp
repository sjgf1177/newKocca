...
<fieldset>
	...
	<form action="/<c:out value="${paramVO.siteName }" />/hkEtcResve/resveSttus/stepSave.do" name="resveForm" id="resveForm" method="post" class="form-inline">
	...
	<input type="hidden" name="resveSn" id="resveSn" value="<c:out value="${result.resveSn }" />" />
		<div class="bdView">
				...
				<ul class="col-12 col-md-9 board_sorting_con">
					<span class="select_box">
							...
							<select class="select_style_0" name="usePurposeSe" id="usePurposeSe" title="사용목적구분을 선택해 주세요." style="width: 150px;">
								...
								<c:forEach var="item" items="${COM149CodeList }">
			...
			<%-- <table class="table table-bordered">
				<caption>인적사항 입력정보 이름, 휴대폰, 이메일, 나이, 직업, 거주지역, 사용인원, 사용목적, 사용목적 상세, 예약정보 수신방법으로 구분되는 표
				</caption>
			  	<colgroup>
		...
		<div class="btnSet row">
			<%-- <div class="col-md-6 tal">
				<a href="/<c:out value="${paramVO.siteName }" />/hkEtcResve/resveSttus/step2.do?menuNo=<c:out value="${param.menuNo }" />&amp;locationTp=prev" class="btnIcPrev">이전</a>
			</div> --%>
			<div class="col-md-12 tar">
				<button type="submit" class="btn btn-black" id="resveRegBtn" style="font-size: 1.4rem">저장</button>
				<%-- <a href="/<c:out value="${paramVO.siteName }" />/hkEtcResve/resveSttus/step1.do?menuNo=<c:out value="${param.menuNo }" />" class="btn btn-gray">취소</a> --%>
				<button type="button" class="btn btn-gray"  style="font-size: 1.4rem" onclick="location.href='/<c:out value="${paramVO.siteName }" />/hkEtcResve/resveSttus/step1.do?menuNo=<c:out value="${param.menuNo }" />';">취소</button>
			</div>
		</div>
		</form>
</fieldset>
