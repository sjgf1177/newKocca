...
<c:if test="${empty result}">
	<c:set var="action" value="/ckl/bbs/${paramVO.bbsId}/insert.do" />
</c:if>
<c:if test="${not empty result}">
	<c:set var="action" value="/ckl/bbs/${paramVO.bbsId}/update.do" />
</c:if>
...
<script type="text/javascript">
	...
	function del(){
		var form = $("#board")[0];
		form.action = "/ckl/bbs/${paramVO.bbsId}/delete.do";
		form.submit();
	}
			...
			<tbody>
				...
				<tr>
					...
					<td>
						...
						<input type="radio" name="secret" id="secret2" value="Y" <c:if test="${result.secret eq 'Y' or empty result.secret}">checked="checked"</c:if>><label for="secret2">비공개</label>
					</td>
				</tr>
				<tr>
...
<div class="btnSet tac">
	<a href="javascript:checkForm();" class="btn btn-primary">확인</a>
	<a href="#" onclick="javascript:history.back(0);return false;" class="btn btn-gray">취소</a>
	<c:if test="${not empty result}" >
