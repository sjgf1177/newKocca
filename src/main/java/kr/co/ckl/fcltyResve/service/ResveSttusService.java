...
import kr.co.ckl.fcltyResve.paging.ResveSttusPageQuery;
...
public class ResveSttusService extends DefaultCmmProgramService {
	...
	public void listFcltySpceNm(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		List<ZValue> list = sqlDao.listDAO("FcltyResveDAO.listFcltySpceNm", param);
	...
	public void listDaly(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		List<ZValue> list = sqlDao.listDAO("FcltyResveDAO.listDalyResveSttus", param);
		int i=0;
		String tmpSn = "";
	...
	public void downloadExcelDaly(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		List<ZValue> resultList = sqlDao.listDAO("FcltyResveDAO.listDalyResveSttus", param);
	...
	public void list(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		super.setCountQueryId("FcltyResveDAO.listResveSttusCnt");
		super.setListQueryId("FcltyResveDAO.listResveSttus");
		super.list(paramCtx);
		...
		List<ZValue> listEqpmnNm = sqlDao.listDAO("FcltyResveDAO.listEqpmnNm", param);
	...
	public void view(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		super.setViewQueryId("FcltyResveDAO.viewResveSttus");
		super.view(paramCtx);

		List<ZValue> list = sqlDao.listDAO("FcltyResveDAO.listEqpmnByResve", param);
	...
	public void resveStep1(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		if (session.getAttribute("resultStep1") != null) {
			...
			if (!"".equals(param.getString("resveSn"))) {
				ZValue resultData = sqlDao.selectDAO("FcltyResveDAO.viewResveSttus", param);
				List<ZValue> eqpmnList = sqlDao.listDAO("FcltyResveDAO.listEqpmnByResve", param);
	...
	public void resveStep2(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		if (!"".equals(param.getString("resveSn"))) {
			ZValue viewVO = sqlDao.selectDAO("FcltyResveDAO.viewResveSttus", param);
	...
	public void resveSave(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		if (!"".equals(param.getString("resveSn"))) {
			sqlDao.updateDAO("FcltyResveDAO.updateResve", resultData);
			sqlDao.deleteDAO("FcltyResveDAO.deleteResveEqpmnByResveSn", param);
		}
		else {
			String resveNo =  RandomStringUtils.random(5, "abcdefghijklmnopqrstuvwxyz1234567890");
			resultData.put("resveNo", resveNo);
			pkValue = sqlDao.insertDAO("FcltyResveDAO.insertResve", resultData);
		}
			...
			for (ZValue eqpmnVO : eqpmnList) {
				...
				if (!"".equals(eqpmnDetailSnStr)) {
					...
					for (String eqpmnDetailSn : eqpmnDetailSns) {
						...
						sqlDao.insertDAO("FcltyResveDAO.insertResveEqpmn", resultData);
					}
				}
			}
		}
		...
		ZValue vo = sqlDao.selectDAO("FcltyResveDAO.viewResveSttus", param);
		...
		if ("01".equals(sendTp) || "03".equals(sendTp)) { //sms
			...
			if (!"".equals(mbtlnum)) {
				...
				String callback = EgovProperties.getProperty("Globals.sms.ckl.fcltyresve.callback");
				param.put("phone", mbtlnum);
		    	param.put("callback", callback);
		    	param.put("msg", msg);

		    	param.put("refkey", "cklab");
				emsService.sndngSms(param);
			}
		}
		if ("02".equals(sendTp) || "03".equals(sendTp)) { //email
			if (!"".equals(vo.getString("email",""))) {
				String title = "[콘텐츠코리아랩]시설예약이 신청되었습니다.";
		    	...
		    	contSb.append("<p style=\"font-size:14px;padding-top:10px\">안녕하세요. <strong>").append( vo.getString("resveNm") ).append("</strong>님! 콘텐츠코리아랩 고객님의 예약정보 입니다.<p>");
		        contSb.append("<ul>");
		...
		StringBuilder url = new StringBuilder("/bos/fcltyResve/resveSttus/resveComplete.do");
	...
	public void resveComplete(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		ZValue result = sqlDao.selectDAO("FcltyResveDAO.viewResveSttus", param);
		List<ZValue> eqpmnList = sqlDao.listDAO("FcltyResveDAO.listEqpmnByResve", param);
	...
	public void updateResveSttus(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		sqlDao.updateDAO("FcltyResveDAO.updateResveSttus", param);
		ZValue vo = sqlDao.selectDAO("FcltyResveDAO.viewResveSttus", param);
		...
		if ("02".equals(sendTp) || "03".equals(sendTp)) { //email
			if (!"".equals(vo.getString("email",""))) {
				String title = "[콘텐츠코리아랩]시설예약상태가 변경되었습니다.";
		    	String email = vo.getString("email");
		    	String content = null;
		    	StringBuffer contSb = new StringBuffer();
	...
	public void delete(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		super.setDeleteQueryId("FcltyResveDAO.deleteResveSttus");
		super.delete(paramCtx);

		String pageQueryString = pageQuery.getPageQueryString(param);
		StringBuilder url = new StringBuilder("/bos/fcltyResve/resveSttus/list.do");
	...
	public void listCal(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		//공휴일 목록
		List<ZValue> hldyList = sqlDao.listDAO("HldyDAO.listHldy", param);

		//시설관리 상세
		ZValue fcltyVO = sqlDao.selectDAO("FcltyResveDAO.viewFclty", param);
	...
	public void listTime(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		List<ZValue> listTime = sqlDao.listDAO("FcltyResveDAO.listResveByCal", param);
	...
	public void listEqpmn(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		List<ZValue> list = sqlDao.listDAO("FcltyResveDAO.listEqpmnByFclty", param);
		List<ZValue> listByDt = sqlDao.listDAO("FcltyResveDAO.listEqpmnByDt", param);
	...
	public void downloadExcel(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		*/
		String[] titles = {"예약번호","내/외부구분","지역구분","위치구분","시설구분","공간명","공간명(영문)","장비","예약자명","예약일자","예약시간","예약상태","휴대폰번호","이메일","나이","직업","거주지","사용인원","예약수신방법","사용목적구분","사용목적","사용여부","발권일시","재발권여부","재발권일시","키오스크위치","시스템등록일"};


		List<ZValue> resultList = sqlDao.listDAO("FcltyResveDAO.listExcelResveSttus", param);
	...
	public void step0(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		int resveCnt = sqlDao.selectCount("FcltyResveDAO.listUserReseveCount", param);
		int penaltyCnt = sqlDao.selectCount("FcltyResveDAO.listUserPenaltyCount", param);

		if (resveCnt >= 3) {
			model.addAttribute(ModelAndViewResolver.RESULT_CODE_KEY, ModelAndViewResolver.ERROR);
			model.addAttribute(ModelAndViewResolver.MSG_KEY, "1일 최대 3회 예약 가능 횟수를 초과하였습니다.");
			return;
		}
	...
	public void stepSave(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		StringBuilder url = new StringBuilder("/"+param.getString("siteName")+"/fcltyResve/resveSttus/step4.do");
	...
	public void saveCmmnt(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		ModelMap model = paramCtx.getModel();
		initCmmnParam(param);
		sqlDao.updateDAO("FcltyResveDAO.updateResveCmmnt", param);
