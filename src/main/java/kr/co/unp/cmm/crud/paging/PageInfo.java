...
public class PageInfo implements IPageInfo
{
	...
	public String getPageNavString( int pageSize, int pageUnit, int total, int cpage, String link )
	{
		...
		if (prev10 > 0)
		{
			sbuf.append("<li class='first'><a style='opacity:1' title=\"처음 페이지로 이동\" href='").append(link).append("&amp;pageIndex=1'>")/*.append("처음 페이지로 이동")*/.append("</a></li>");
			sbuf.append("<li class='prev'><a style='opacity:1' title=\"이전 "+pageSize+"페이지  목록으로 이동\" href='").append(link).append("&amp;pageIndex=").append(prev10).append("'>")/*.append("이전 "+pageSize+"페이지  목록으로 이동")*/.append("</a></li>");
		} // end if 이전10개
		else
		{
		...
		for (int i = 1 + prev10; i < next10 && i <= totalPage; i++)
		{
			...
			{
				sbuf.append("<li><a title='"+i+"페이지로 이동' href='").append(link).append("&amp;pageIndex=").append(i).append("'>").append(i).append("</a></li>");
			} // end if 현재페이지 링크제거
		} // end for
		...
		if (totalPage >= next10)
		{
			sbuf.append("<li class='next'><a style='opacity:1' title=\"다음 "+nextCnt+"페이지 목록으로 이동\" href='").append(link).append("&amp;pageIndex=").append(next10).append("'>")/*.append("다음 "+nextCnt+"페이지 목록으로 이동")*/.append("</a></li>");
			sbuf.append("<li class='end'><a style='opacity:1' title=\"마지막 페이지로 이동\" href='").append(link).append("&amp;pageIndex=").append(totalPage).append("'>")/*.append("마지막 페이지로 이동")*/.append("</a></li>");
		} // end if 다음10개
		else
		{
