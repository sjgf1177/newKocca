...
import kr.co.edu.hkEtcResve.paging.ResveSttusPageQuery;
...
public class ResveSttusService extends DefaultCmmProgramService {
	...
	public void listFcltySpceNm(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		List<ZValue> list = sqlDao.listDAO("HkEtcResveDAO.listFcltySpceNm", param);
	...
	public void listDaly(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		List<ZValue> list = sqlDao.listDAO("HkEtcResveDAO.listDalyResveSttus", param);
		int i=0;
		String tmpSn = "";
	...
	public void downloadExcelDaly(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		List<ZValue> resultList = sqlDao.listDAO("HkEtcResveDAO.listDalyResveSttus", param);
	...
	public void list(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		super.setCountQueryId("HkEtcResveDAO.listResveSttusCnt");
		super.setListQueryId("HkEtcResveDAO.listResveSttus");
		super.list(paramCtx);
		...
		List<ZValue> listEqpmnNm = sqlDao.listDAO("HkEtcResveDAO.listEqpmnNm", param);
	...
	public void view(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		super.setViewQueryId("HkEtcResveDAO.viewResveSttus");
		super.view(paramCtx);

		List<ZValue> list = sqlDao.listDAO("HkEtcResveDAO.listEqpmnByResve", param);
	...
	public void resveStep1(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		if (session.getAttribute("resultStep1") != null) {
			...
			if (!"".equals(param.getString("resveSn"))) {
				ZValue resultData = sqlDao.selectDAO("HkEtcResveDAO.viewResveSttus", param);
				List<ZValue> eqpmnList = sqlDao.listDAO("HkEtcResveDAO.listEqpmnByResve", param);
	...
	public void resveStep2(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		if (!"".equals(param.getString("resveSn"))) {
			ZValue viewVO = sqlDao.selectDAO("HkEtcResveDAO.viewResveSttus", param);
	...
	public void resveSave(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		if (!"".equals(param.getString("resveSn"))) {
			sqlDao.updateDAO("HkEtcResveDAO.updateResve", resultData);
			sqlDao.deleteDAO("HkEtcResveDAO.deleteResveEqpmnByResveSn", param);
		}
		else {
			String resveNo =  RandomStringUtils.random(5, "abcdefghijklmnopqrstuvwxyz1234567890");
			resultData.put("resveNo", resveNo);
			pkValue = sqlDao.insertDAO("HkEtcResveDAO.insertResve", resultData);
		}
			...
			for (ZValue eqpmnVO : eqpmnList) {
				...
				if (!"".equals(eqpmnDetailSnStr)) {
					...
					for (String eqpmnDetailSn : eqpmnDetailSns) {
						...
						sqlDao.insertDAO("HkEtcResveDAO.insertResveEqpmn", resultData);
					}
				}
			}
		}
		...
		ZValue vo = sqlDao.selectDAO("HkEtcResveDAO.viewResveSttus", param);
		...
		if ("01".equals(sendTp) || "03".equals(sendTp)) { //sms
			...
			if (!"".equals(mbtlnum)) {
				String msg = "["+vo.getString("resveNo")+"]"+  vo.getString("resveDt") +" 예약이 " + "신청되었습니다.";
				if ("03".equals(vo.getString("resveSttus"))) {
					msg = "["+vo.getString("resveNo")+"]"+  vo.getString("resveDt") +" 대기예약이 " + "신청되었습니다.";
				}
		...
		if ("02".equals(sendTp) || "03".equals(sendTp)) { //email
			if (!"".equals(vo.getString("email",""))) {
		        ...
		        contSb.append("<li style=\"line-height:30px\"><strong>장비명</strong> :&nbsp; <span style=\"color:#5c82e8;\">").append( vo.getString("eqpmnNm") ).append( "</span></li><br/>");
		...
		StringBuilder url = new StringBuilder("/bos/hkEtcResve/resveSttus/resveComplete.do");
	...
	public void resveComplete(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		ZValue result = sqlDao.selectDAO("HkEtcResveDAO.viewResveSttus", param);
		List<ZValue> eqpmnList = sqlDao.listDAO("HkEtcResveDAO.listEqpmnByResve", param);
	...
	public void updateResveSttus(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		sqlDao.updateDAO("HkEtcResveDAO.updateResveSttus", param);
		ZValue vo = sqlDao.selectDAO("HkEtcResveDAO.viewResveSttus", param);
		...
		if ("01".equals(sendTp) || "03".equals(sendTp)) { //sms
			String mbtlnum = vo.getString("mbtlnum","").replaceAll("-", "");
			if (!"".equals(mbtlnum)) {
				String msg = "["+vo.getString("resveNo")+"]"+  vo.getString("resveDt") +" 예약이 " + vo.getString("resveSttusNm") + "되었습니다.";
				String callback = EgovProperties.getProperty("Globals.sms.edu.fcltyresve.callback");
				param.put("phone", mbtlnum);
		    	param.put("callback", callback);
		    	param.put("msg", msg);
	...
	public void delete(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		super.setDeleteQueryId("HkEtcResveDAO.deleteResveSttus");
		super.delete(paramCtx);

		String pageQueryString = pageQuery.getPageQueryString(param);
		StringBuilder url = new StringBuilder("/bos/hkEtcResve/resveSttus/list.do");
	...
	public void listCal(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		//시설관리 상세
		//ZValue fcltyVO = sqlDao.selectDAO("HkEtcResveDAO.viewFclty", param);
	...
	public void listTime(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		/*List<ZValue> listTime = sqlDao.listDAO("HkEtcResveDAO.listResveByCal", param);
	...
	public void listEqpmn(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		List<ZValue> list = sqlDao.listDAO("HkEtcResveDAO.listEqpmnByFclty", param);
		List<ZValue> listByDt = sqlDao.listDAO("HkEtcResveDAO.listEqpmnByDt", param);
	...
	public void downloadExcel(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		*/
		String[] titles = {"예약번호","장비","예약자명","예약일자","예약시간","예약상태","휴대폰번호","이메일","나이","직업","거주지","사용인원","예약수신방법","사용목적구분","사용목적","사용여부","시스템등록일"};


		List<ZValue> resultList = sqlDao.listDAO("HkEtcResveDAO.listExcelResveSttus", param);
	...
	public void step0(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		/*int resveCnt = sqlDao.selectCount("HkEtcResveDAO.listUserReseveCount", param);*/
		int penaltyCnt = sqlDao.selectCount("HkEtcResveDAO.listUserPenaltyCount", param);
	...
	public void step3(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		int resve_Dt_Cnt = sqlDao.selectCount("HkEtcResveDAO.listUserReseveCount", param);
		...
		int today_resveCnt = sqlDao.selectCount("HkEtcResveDAO.listUserReseveCount", param);
	...
	public void stepSave(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		String msg = "["+vo.getString("resveNo")+"]"+  vo.getString("resveDt")+"," +vo.getString("resveNm") +" 예약  ";
		...
		StringBuilder url = new StringBuilder("/"+param.getString("siteName")+"/hkEtcResve/resveSttus/step4.do");
	...
	public void cancelResveSttus(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		if ("01".equals(sendTp) || "03".equals(sendTp)) { //sms
			String mbtlnum = vo.getString("mbtlnum","").replaceAll("-", "");
			if (!"".equals(mbtlnum)) {
				String msg = "["+vo.getString("resveNo")+"]"+  vo.getString("resveDt") +" 예약이 " + "취소되었습니다.";
				if ("03".equals(vo.getString("resveSttus"))) {
					msg = "["+vo.getString("resveNo")+"]"+  vo.getString("resveDt") +" 대기예약이 " + "취소되었습니다.";
				}
				String callback = EgovProperties.getProperty("Globals.sms.edu.fcltyresve.callback");
				param.put("phone", mbtlnum);
		    	param.put("callback", callback);
		...
		if ("02".equals(sendTp) || "03".equals(sendTp)) { //email
			if (!"".equals(vo.getString("email",""))) {
		    	...
		    	String content = null;
		    	StringBuffer contSb = new StringBuffer();
		    	contSb.append("<h2 style=\"font-size:30px;font-weight:600\">장비예약 변경정보</h2>");
		    	contSb.append("<p style=\"font-size:14px;padding-top:10px\">안녕하세요. <strong>").append( vo.getString("resveNm") ).append("</strong>님! EDUKOCCA 고객님의 예약정보 입니다.<p>");
		        contSb.append("<ul>");
		        contSb.append("<li style=\"line-height:30px\"><strong>예약번호</strong> :&nbsp; <span style=\"color:#5c82e8;\">").append( vo.getString("resveNo") ).append( "</span></li><br/>");
		        contSb.append("<li style=\"line-height:30px\"><strong>예약일시</strong> :&nbsp; <span style=\"color:#5c82e8;\">").append( vo.getString("resveDt") ).append(" ").append(vo.getString("resveBeginTime")).append(":00 ~ ").append(vo.getString("resveEndTime")).append( ":59 까지</span></li><br/>");
	...
	public void saveCmmnt(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		ModelMap model = paramCtx.getModel();
		initCmmnParam(param);
		sqlDao.updateDAO("HkEtcResveDAO.updateResveCmmnt", param);
	...
	public void chgResveSn(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		for(String resveSn : resveSn_arr){
			param.put("resveSn", resveSn);
			sqlDao.updateDAO("HkEtcResveDAO.chgResveSn", param);
		}
	 ...
	 * @throws Exception
	 */
	public void resveStats(ParameterContext<ZValue> paramCtx) throws Exception {
		ZValue param = paramCtx.getParam();
		ModelMap model = paramCtx.getModel();
		initCmmnParam(param);
		...
		List<ZValue> datelist = sqlDao.listDAO("HkEtcResveDAO.listResveStatsDates", param);
		model.addAttribute("dateList",datelist);
		
		List<ZValue> list = sqlDao.listDAO("HkEtcResveDAO.listResveStats", param);
		model.addAttribute("resultList",list);
	}
	 ...
	 * @throws Exception
	 */
	public void downloadExcelResveStats(ParameterContext<ZValue> paramCtx) throws Exception {
		ZValue param = paramCtx.getParam();
		ModelMap model = paramCtx.getModel();
		initCmmnParam(param);

		SimpleDateFormat sf = new SimpleDateFormat("yyyy-MM-dd");
		SimpleDateFormat sf_file = new SimpleDateFormat("yyyyMMdd");
		Calendar cal = Calendar.getInstance();
		if ("".equals(param.getString("resveSdate",""))) param.put("resveSdate",sf.format(cal.getTime()));
		if ("".equals(param.getString("resveEdate",""))) param.put("resveEdate",sf.format(cal.getTime()));

		String[] titles = {"장비번호","장비명","일자","이용시간H","예약완료","승인대기","예약취소"};

		List<ZValue> list = sqlDao.listDAO("HkEtcResveDAO.listResveStats", param);
		ExcelGenerateVO vo = new ExcelGenerateVO("장비예약통계_"+sf_file.format(cal.getTime()), titles, list);
		model.addAttribute("excel", vo);
