package kr.co.edu.onlineEdu.library;
...
public class KISA_SEED_CBC {
	...
	// 암호화 대칭 키
	private static final byte pbUserKey[]  = { (byte) 0x5c, (byte) 0x11, (byte) 0x19, (byte) 0x1d,
											   (byte) 0x1f, (byte) 0x17, (byte) 0x12, (byte) 0x18,
								  			   (byte) 0x11, (byte) 0x19, (byte) 0x1a, (byte) 0x2f,
								  			   (byte) 0x10, (byte) 0x14, (byte) 0x1b, (byte)0x16};
	// CBC 대칭 키
	private static final byte bszIV[] = {	(byte)0x27,  (byte)0x28,  (byte)0x27, (byte) 0x6d,
...
//                (byte)0x08, (byte)0x09, (byte)0x0A, (byte)0x0B, (byte)0x0C, (byte)0x0D, (byte)0x0E, (byte)0x0F};
	    ...
	    byte[] defaultCipherText = SEED_CBC_Encrypt( pbData, 0, PLAINTEXT_LENGTH);


	    byte[] PPPPP = SEED_CBC_Decrypt(defaultCipherText, 0, CIPHERTEXT_LENGTH);
	    ...
	    byte[] Cipher1 = SEED_CBC_Encrypt(pbData1,0, 2);

	    byte[] Plain1 = SEED_CBC_Decrypt(Cipher1, 0, 16);
	    ...
	    byte[] Cipher2 = SEED_CBC_Encrypt(pbData2,0, 16);

	    byte[] Plain2 = SEED_CBC_Decrypt(Cipher2, 0, 32);
	    ...
	    byte[] Cipher3 = SEED_CBC_Encrypt(pbData3, 0, 18);

	    byte[] Plain3 = SEED_CBC_Decrypt(Cipher3, 0, 32);
	...
	//암호화 처리
	public static String CKLTREE_SEED_Encrypt(String message) {
		String result ="";
		String temp = message;
		...
		try {
			...
			//암호화 시작
			byte[] enc = SEED_CBC_Encrypt(temp.getBytes("utf-8"), 0, temp.getBytes("utf-8").length);
			//BASE64인코딩
			result = Enc.encode(enc);
	...
	//복호화 처리
	public static String CKLTREE_SEED_CBC_Decrypt(String message) {
		String result = "";
		String msg = "";
		msg = message.replace("%2B", "+");

		try {
			sun.misc.BASE64Decoder Dnc = new sun.misc.BASE64Decoder();
			//받은 파라미터 값은 먼저 디코딩
			byte[] temp = Dnc.decodeBuffer(msg);
			//복호화 시작
			byte[] dec = SEED_CBC_Decrypt(temp, 0, temp.length);
			//문자열로 리턴
			result = new String(dec, "utf-8");
//			result = result.replace("&#43;", "+");


//			result = new String(dec, "utf-8").replaceAll("%252b", "+");
