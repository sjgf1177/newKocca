...
public class CultureResveSttusService extends DefaultCmmProgramService {

	Logger log = Logger.getLogger(this.getClass());

	public CultureResveSttusService() {
	...
	public void listFcltySpceNm(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		initCmmnParam(param);
		List<ZValue> list = sqlDao.listDAO("HkCultureResveDAO.listFcltySpceNm", param);
	...
	public void listDaly(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		List<ZValue> list = sqlDao.listDAO("HkCultureResveDAO.listDalyResveSttus", param);
		int i=0;
		String tmpSn = "";
	...
	public void downloadExcelDaly(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		List<ZValue> resultList = sqlDao.listDAO("HkCultureResveDAO.listDalyResveSttus", param);
		ExcelGenerateVO vo = new ExcelGenerateVO("문화광장 일별예약현황 목록", titles, resultList);
	...
	public void list(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		super.setCountQueryId("HkCultureResveDAO.listResveSttusCnt");
		super.setListQueryId("HkCultureResveDAO.listResveSttus");
		super.list(paramCtx);
	...
	public void view(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		super.setViewQueryId("HkCultureResveDAO.viewResveSttus");
		super.view(paramCtx);

		List<ZValue> list = sqlDao.listDAO("HkCultureResveDAO.listEqpmnByResve", param);
	...
	public void resveStep1(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		if (session.getAttribute("resultStep1") != null) {
			...
			if (!"".equals(param.getString("resveSn"))) {
				ZValue resultData = sqlDao.selectDAO("HkCultureResveDAO.viewResveSttus", param);
				List<ZValue> eqpmnList = sqlDao.listDAO("HkCultureResveDAO.listEqpmnByResve", param);
	...
	public void resveStep2(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		if (!"".equals(param.getString("resveSn"))) {
			ZValue viewVO = sqlDao.selectDAO("HkCultureResveDAO.viewResveSttus", param);
	...
	public void resveSave(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		if (!"".equals(param.getString("resveSn"))) {
			sqlDao.updateDAO("HkCultureResveDAO.updateResve", resultData);
			sqlDao.deleteDAO("HkCultureResveDAO.deleteResveEqpmnByResveSn", param);
		}
		else {
			String resveNo =  RandomStringUtils.random(5, "abcdefghijklmnopqrstuvwxyz1234567890");
			resultData.put("resveNo", resveNo);
			pkValue = sqlDao.insertDAO("HkCultureResveDAO.insertResve", resultData);
		}
			...
			for (ZValue eqpmnVO : eqpmnList) {
				...
				if (!"".equals(eqpmnDetailSnStr)) {
					...
					for (String eqpmnDetailSn : eqpmnDetailSns) {
						...
						sqlDao.insertDAO("HkCultureResveDAO.insertResveEqpmn", resultData);
					}
				}
			}
		}
		...
		ZValue vo = sqlDao.selectDAO("HkCultureResveDAO.viewResveSttus", param);
		...
		if ("02".equals(sendTp) || "03".equals(sendTp)) { //email
			if (!"".equals(vo.getString("email",""))) {
				String title = "[EDUKOCCA]문화광장 시설예약이 신청되었습니다.";
		    	String email = vo.getString("email");
		    	String content = null;
		    	StringBuffer contSb = new StringBuffer();
		    	contSb.append("<h2 style=\"font-size:30px;font-weight:600\">문화광장 시설예약 신청정보</h2>");
		...
		StringBuilder url = new StringBuilder("/bos/hkCultureResve/resveSttus/resveComplete.do");
	...
	public void resveComplete(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		ZValue result = sqlDao.selectDAO("HkCultureResveDAO.viewResveSttus", param);
		List<ZValue> eqpmnList = sqlDao.listDAO("HkCultureResveDAO.listEqpmnByResve", param);
	...
	public void updateResveSttus(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		sqlDao.updateDAO("HkCultureResveDAO.updateResveSttus", param);
		ZValue vo = sqlDao.selectDAO("HkCultureResveDAO.viewResveSttus", param);
		...
		if ("02".equals(sendTp) || "03".equals(sendTp)) { //email
			if (!"".equals(vo.getString("email",""))) {
				String title = "[EDUKOCCA]문화광장 시설예약상태가 변경되었습니다.";
		    	String email = vo.getString("email");
		    	String content = null;
		    	StringBuffer contSb = new StringBuffer();
		    	contSb.append("<h2 style=\"font-size:30px;font-weight:600\">문화광장 시설예약 변경정보</h2>");
		    	contSb.append("<p style=\"font-size:14px;padding-top:10px\">안녕하세요. <strong>").append( vo.getString("resveNm") ).append("</strong>님! EDUKOCCA 고객님의 예약정보 입니다.<p>");
		        contSb.append("<ul>");
		        contSb.append("<li style=\"line-height:30px\"><strong>예약번호</strong> :&nbsp; <span style=\"color:#5c82e8;\">").append( vo.getString("resveNo") ).append( "</span></li><br/>");
		        contSb.append("<li style=\"line-height:30px\"><strong>예약일시</strong> :&nbsp; <span style=\"color:#5c82e8;\">").append( vo.getString("resveDt") ).append(" ").append(vo.getString("resveBeginTime")).append(":00 ~ ").append(vo.getString("resveEndTime")).append( ":59 까지</span></li><br/>");
	...
	public void delete(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		super.setDeleteQueryId("HkCultureResveDAO.deleteResveSttus");
		super.delete(paramCtx);

		String pageQueryString = pageQuery.getPageQueryString(param);
		StringBuilder url = new StringBuilder("/bos/hkCultureResve/resveSttus/list.do");
	...
	public void listCal(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		//시설관리 상세
		ZValue fcltyVO = sqlDao.selectDAO("HkCultureResveDAO.viewFclty", param);
	...
	public void listTime(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		List<ZValue> listTime = sqlDao.listDAO("HkCultureResveDAO.listResveByCal", param);
	...
	public void listEqpmn(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		List<ZValue> list = sqlDao.listDAO("HkCultureResveDAO.listEqpmnByFclty", param);
		List<ZValue> listByDt = sqlDao.listDAO("HkCultureResveDAO.listEqpmnByDt", param);
	...
	public void downloadExcel(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		*/
		String[] titles = {"예약번호","내/외부구분","지역구분","위치구분","시설구분","공간명","공간명(영문)","단체명","예약일자","예약시간","예약상태","연락처","이메일","거주지","사용인원","예약수신방법","사용목적구분","사용목적","사용여부","시스템등록일"};

		List<ZValue> resultList = sqlDao.listDAO("HkCultureResveDAO.listExcelResveSttus", param);
		...
		ExcelGenerateVO vo = new ExcelGenerateVO("문화광장 예약현황 목록", titles, resultList);
		model.addAttribute("excel", vo);
	...
	public void step0(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		int penaltyCnt = sqlDao.selectCount("HkCultureResveDAO.listUserPenaltyCount", param);
	...
	public void step3(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		int resve_Dt_Cnt = sqlDao.selectCount("HkCultureResveDAO.listUserReseveCount", param);
		...
		int today_resveCnt = sqlDao.selectCount("HkCultureResveDAO.listUserReseveCount", param);
	...
	public void stepSave(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		StringBuilder url = new StringBuilder("/"+param.getString("siteName")+"/hkCultureResve/resveSttus/step4.do");
	...
	public void saveCmmnt(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		ModelMap model = paramCtx.getModel();
		initCmmnParam(param);
		sqlDao.updateDAO("HkCultureResveDAO.updateResveCmmnt", param);
	...
	public void chgResveSn(ParameterContext<ZValue> paramCtx) throws Exception {
		...
		for(String resveSn : resveSn_arr){
			param.put("resveSn", resveSn);
			sqlDao.updateDAO("HkCultureResveDAO.chgResveSn", param);
		}
