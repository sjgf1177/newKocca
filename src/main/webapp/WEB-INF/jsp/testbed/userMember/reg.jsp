...
<script type="text/javascript">
	...
	$(function(){
		...
		crtfcPopup = function(authSe) { // 이메일/휴대폰 인증 팝업창
			...
			window.open("/testbed/userMember/crtfcPopup.do?viewType=BODY&authSe="+authSe+"&val="+val+"&mode=FU&authSeVal="+authSeVal, "crtfcPopup", "top=300, left=600, width=568, height="+height+", status=yes, resizable=yes, scrollbars=no");
		};
		...
		$("#emailAuthDel").click(function(){
			...
			$.post(
				"/testbed/userMember/authSeChange.json",
		...
		$("#chgMemTpBtn").click(function() {
			...
			if (confirm("14세 이상 회원으로 전환하시겠습니까?")) {
				var url = "/testbed/userMember/updateParntsAgreChange.json";
				var params = {};
...
<fieldset>
	...
	<form method="post" name="agreeForm" id="agreeForm" action="/testbed/userMember/update.do" >
	...
	<input type="hidden" name="menuNo" value="<c:out value='${paramVO.menuNo}'/>"/>
					...
					<tbody>
							...
							<td>
								<c:set var="data" value="${fn:split(result.email,'@')}" />
									<span class="input_style_1_con">
										<input name="email1" id="email1" type="text" title="이메일 아이디" value="${data[0]}" class="w20p input_style_1"/>
									</span>
									@
									<span class="input_style_1_con">
										<input name="email2" id="email2" type="text" title="이메일 주소" value="${data[1]}" readonly="readonly" class="w20p emaildomain_form input_style_1"/>
									</span>
						...
						<tr>
							...
							<td>
								...
								-
								<span class="input_style_1_con">
									<input type="text" id="mbtlnum2" name="mbtlnum2" value="${data[1]}" class="w20p input_style_1" title="휴대폰 중간자리" maxlength="4" onkeyup="this.value=check_txt(this.value)"/>
								</span>
								-
								<span class="input_style_1_con">
									<input type="text" id="mbtlnum3" name="mbtlnum3" value="${data[2]}" class="w20p input_style_1" title="휴대폰 끝자리" maxlength="4" onkeyup="this.value=check_txt(this.value)"/>
								</span>
						...
						<tr>
							<th scope="row"><span class="red_star_text"><span class="sr-only" style="display:none;">(필수입력)</span></span> 인재캠퍼스 정보수신 여부</th>
							<td>서비스 및 이벤트 소식을<br>
						...
						<tr><!-- 콘텐츠문화광장 -->
							<th scope="row"><span class="red_star_text"><span class="sr-only" style="display:none;">(필수입력)</span></span> 콘텐츠문화광장 정보수신 여부</th>
							<td>서비스 및 이벤트 소식을<br>
								<span class="radio_box_con">
		...
		<div class="btnSet row">
			...
			<div class="col-md-6 board_util_btn_con">
