...
<div class="btnSet tac">
	<c:if test="${progrsSttus eq PROGRESS}">
		<a href="/edu/progrm/applcnt/reg01.do?prgSn=<c:out value='${result.prgSn}'/>&amp;sttus=02&amp;${pageQueryString}" onclick="return confirm('신청하시겠습니까?');" class="btn btn-primary">신청하기</a>
	</c:if>
	<c:if test="${progrsSttus eq FINISH_PSNCPA}">
		<a href="/edu/progrm/applcnt/reg01.do?prgSn=<c:out value='${result.prgSn}'/>&amp;sttus=01&amp;${pageQueryString}" onclick="return confirm('대기접수하시겠습니까?');" class="btn btn-primary">대기접수하기</a>
	</c:if>
	<%-- <c:if test="${result.progrsSttus eq PROGRESS}">
		<c:if test="${result.psncpa > result.totCnt02}">
			<a href="/edu/progrm/applcnt/reg01.do?prgSn=<c:out value='${result.prgSn}'/>&amp;sttus=02&amp;${pageQueryString}" onclick="return confirm('신청/접수하시겠습니까?');" class="btn btn-primary">신청/접수하기</a>
		</c:if>
		<c:if test="${result.waitReqstAt eq 'Y' and result.reqstMthd eq '01'}">
			<c:if test="${result.psncpa <= result.totCnt02 and result.waitReqstNmpr > result.totCnt01}">
				<a href="/edu/progrm/applcnt/reg01.do?prgSn=<c:out value='${result.prgSn}'/>&amp;sttus=01&amp;${pageQueryString}" onclick="return confirm('대기 신청/접수하시겠습니까?');" class="btn btn-primary">대기 신청/접수하기</a>
			</c:if>
		</c:if>
	</c:if> --%>
	<a href="/edu/progrm/master/list.do?${pageQueryString}" class="fr btn btn-primary">목록</a>
</div>
