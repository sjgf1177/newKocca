...
public String removeTag(String html) throws Exception {
	return html.replaceAll("<(/)?([a-zA-Z|가-힣]*)(\\s[a-zA-Z|가-힣]*=[^>]*)?(\\s)*(/)?>", "").replaceAll("&nbsp;"," ");
}
%>
<ccc:constantsMap className="kr.co.unp.bbs.vo.SearchVO" var="SearchVO"/>
	...
	<div class="tab_style_1_con">
		<ul class="tab_style_1">
			<li><a href="/edu/main/contents.do?menuNo=500175"><span>소개</span></a></li>
			<li class="active"><a href="/edu/bbs/B0000058/list.do?menuNo=500176" title="현재탭"><span>크리에이터 소식</span></a></li>
		</ul>
	</div>
				...
				<a href="/<c:out value='${paramVO.siteName }' />/bbs/<c:out value='${paramVO.bbsId}' />/view.do?nttId=<c:out value='${result.nttId}'/><c:out value='${pageQueryString}'  escapeXml='false'/> " class="col-12 show fn fieldstory_card">
					<div class="img_box">
						<c:if test="${not empty fileVO}">
							<img alt="[${result.nttSj } ${fileVO.orignlFileNm }" src='/cmm/fms/getImage.do?atchFileId=${fileVO.atchFileId}&amp;fileSn=${fileVO.fileSn}'   />
						</c:if>
					</div>
					<div class="text_box">
						<h5>
