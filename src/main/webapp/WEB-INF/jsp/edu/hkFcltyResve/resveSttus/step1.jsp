...
$(function() {
	...
	$("#nextBtn").click(function() {
		...
		if (confirm("다음 단계로 이동하시겠습니까?")) {
			window.location.href = "/<c:out value="${paramVO.siteName}" />/hkFcltyResve/resveSttus/step2.do?menuNo=<c:out value="${param.menuNo}" />&fcltySn=<c:out value="${param.fcltySn}" />";
		}
		return false;
	});
...
<ul>
	...
	<li>4. 동의 거부 시 불이익에 관한 사항 : 수집 &middot; 이용에 관한 사항의 동의를 거부할 때에는 예약 서비스의 이용이 제한됩니다.   단, 선택정보 사항을 획득하지 못한 사유로 인하여 서비스 제공을 거부하지 않습니다.</li>
			...
			<div class="agreeScrBtm tar">
				<!-- <span class="dib fl">	위 개인정보 수집 및 이용에 </span> -->
