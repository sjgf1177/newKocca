		...
		<c:choose>
			<c:when test="${param.key eq 'nm'}">
				...
				<c:if test="${empty param.mode}">
					<input type="text" name="infoData${idx}" id="infoData${idx}" value='<c:out value="${empty info.nm ? user.userNm : info.nm}"/>' readonly="readonly" title="이름을 입력하세요.">
				</c:if>
			</c:when>
			<c:when test="${param.key eq 'brthdy'}">
				...
				<c:set var="brthdy"><fmt:formatDate value="${brthdy}" pattern="yyyy-MM-dd"/></c:set>
					<input type="text" name="infoData${idx}" id="infoData${idx}" class="sdate maskdate" title="생년월일을 입력하세요." style="width:130px" value='<c:out value="${empty info.brthdy ? brthdy : info.brthdy}"/>' />
				</c:if>
			</c:when>
			...
			<c:when test="${param.key eq 'psitn'}">
				...
				<c:if test="${empty param.mode}">
					<input type="text" name="infoData${idx}" id="infoData${idx}" style="width:110px" value='<c:out value="${info.psitn}"/>' title="소속을 입력하세요." />
				</c:if>
			</c:when>
		</c:choose>
	</td>
