...
<script type="text/javascript">
	...
	$(function(){
		...
		crtfcPopup = function(authSe) { // 이메일/휴대폰 인증 팝업창
			...
			window.open("/ckl/userMember/crtfcPopup.do?viewType=BODY&authSe="+authSe+"&val="+val+"&mode=FU&authSeVal="+authSeVal, "crtfcPopup", "top=300, left=600, width=568, height="+height+", status=yes, resizable=yes, scrollbars=no");
		};
		...
		$("#emailAuthDel").click(function(){
			...
			$.post(
				"/ckl/userMember/authSeChange.json",
...
<fieldset>
	...
	<form method="post" name="agreeForm" id="agreeForm" action="/ckl/userMember/update.do" >
		...
		<div>
			...
			<div class="bdView">
				<table class="table table-bordered">
					...
					<tbody>
						<tr>
							<th scope="row"><label for="email1"><span class="hide_star"><span class="sr-only">(필수입력)</span></span> 이메일</label></th>
							<td>
								<c:set var="data" value="${fn:split(result.email,'@')}" />
									<input name="email1" id="email1" type="text" title="이메일 아이디" value="${data[0]}" class="w20p" />
									@
									<input name="email2" id="email2" type="text" title="이메일 주소" value="${data[1]}" readonly="readonly" class="w20p emaildomain_form" />
									...
									<c:choose>
										<c:when test="${result.authEmailAt eq 'Y'}">
											<a href="#self" class="btn btn-success btn-sm" id="emailAuthCmt">인증완료</a>
											<a href="#self" class="btn btn-danger btn-sm" id="emailAuthDel">인증삭제</a>
										</c:when>
										<c:otherwise>
						...
						<tr>
							...
							<td>
								...
								-
								<input type="text" id="mbtlnum2" name="mbtlnum2" value="${data[1]}" class="w20p" title="휴대폰 중간자리" maxlength="4" onkeyup="this.value=check_txt(this.value)"/>
								-
								<input type="text" id="mbtlnum3" name="mbtlnum3" value="${data[2]}" class="w20p" title="휴대폰 끝자리" maxlength="4" onkeyup="this.value=check_txt(this.value)"/>
								...
								<c:choose>
									<c:when test="${result.authSe eq '02' or result.authSe eq '03' }">
										...
										<a href="#self" class="btn btn-success btn-sm" id="mbtlnumAuthCmt">인증완료</a>
										<a href="#self" class="btn btn-danger btn-sm" id="mbtlnumAuthDel">인증삭제</a>
									</c:when>
									<c:otherwise>
						...
						<tr>
							...
							<td>서비스 및 이벤트 소식을<br>
								...
								<label for="emailAt1"><input type="radio" name="emailAt" value="Y" id="emailAt1" <c:if test="${result.emailAt eq 'Y' }">checked="checked"</c:if>> Email로 수신하겠습니다. </label>
								<label for="emailAt2"><input type="radio" name="emailAt" value="N" id="emailAt2"<c:if test="${result.emailAt eq 'N' }">checked="checked"</c:if>> Email로 수신하지 않겠습니다. </label><br>
								<label for="smsAt1"><input type="radio" name="smsAt" value="Y" id="smsAt1" <c:if test="${result.smsAt eq 'Y' }">checked="checked"</c:if>> SMS로 수신하겠습니다. </label>
								<label for="smsAt2"><input type="radio" name="smsAt" value="N" id="smsAt2"<c:if test="${result.smsAt eq 'N' }">checked="checked"</c:if>> SMS로 수신하지 않겠습니다. </label><br><br>
				...
				<table class="table table-bordered">
					...
					<tbody>
						<tr>
							<th scope="row">거주지</th>
							<td>
								<input type="radio" name="resdncSe" value="01" id="resdncSe1" <c:if test="${result.resdncSe eq '01'}">checked="checked"</c:if>><label for="resdncSe1"> 국내 </label>
								<input type="radio" name="resdncSe" value="02" id="resdncSe2" <c:if test="${result.resdncSe eq '02'}">checked="checked"</c:if>><label for="resdncSe2"> 해외 </label>
								...
								<input type="text" name="resdncOvsea" id="resdncOvsea" value="<c:out value="${result.resdncOvsea}"/>" title="해외인 경우 입력해주세요.">
							</td>
						</tr>
						<tr>
