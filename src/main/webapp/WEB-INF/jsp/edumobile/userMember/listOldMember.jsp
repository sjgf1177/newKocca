	...
	<div>
		<div class="boxGray">
			<p class="fs1 mt10 mb10">한국콘텐츠진흥원에서 운영 중인 웹사이트에 <strong class="fcBlack"><c:out value="${user.userNm}"/>님</strong>의 정보로 추정되는 <strong class="fcBlack">회원정보가 존재합니다.</strong><br/>
				창의인재양성 웹사이트(콘텐츠코리아 랩, 한국콘텐츠아카데미)에서는 아래의 3개 사이트를 통합하면서 중복 회원정보를 정리하고 있습니다. </p>
		</div>
...
<form id="frm" name="frm" method="post" action="/edumobile/userMember/oldMember.do?menuNo=<c:out value="${param.menuNo}"/>" >
	<input type="hidden" name="rm" value='<c:out value="${rm}"/>'/>
	<div class="bdList">
		<table class="table mb0">
			<caption>
				(콘텐츠아카데미) 선택, 번호, 사이트 구분, 아이디(이메일), 성명, 가입일시, 현재상태, 결과 정보를 포함하는 표
			</caption>
			<colgroup>
			<col style="width:8%;">
			<col style="width:8%;">
			...
			<tbody>
				<c:forEach var="result" items="${listOldMember}" varStatus="status">
					<tr>
						...
						<td>
							<c:choose>
								<c:when test="${result.siteSe eq 'kocca'}">콘텐츠아카데미</c:when>
