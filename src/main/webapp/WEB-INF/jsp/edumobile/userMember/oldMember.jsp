				...
				if (data.resultCode == "success") {
					if (data.emailCnt > 0) {
						if (confirm("이미 등록된 이메일입니다. 아이디 찾기 페이지로 이동하시겠습니까?")) {
							location.href = "/edumobile/userMember/idSearch.do?menuNo=${idMenuNo}";
						}
						flag = false;
					}
				}
				...
				if (data.resultCode == "success") {
					if (data.mbtlnumCnt > 0) {
						if (confirm("이미 등록된 휴대폰번호입니다. 아이디 찾기 페이지로 이동하시겠습니까?")) {
							location.href = "/edumobile/userMember/idSearch.do?menuNo=${idUrl}";
						}
						flag = false;
					}
				}
...
//]]>
	...
	<form id="frm" name="frm" method="post" action="/edumobile/userMember/insertOldMember.do" >
								...
								<td>
									<div class="calendarzone">
										<div id="startDate-lry" class="calendarSLayer">
											<iframe id="startDate-ifrm" name="startDate-ifrm" class="calendar-frame" src="/edumobile/html/incFrCalendar.html" title="시작 날짜입력 프레임" frameborder="0" scrolling="no"></iframe>
										</div>
									</div>
