			...
			<form action="/<c:out value="${paramVO.siteName }"/>/bbs/${paramVO.bbsId}/list.do" class="form-inline" method="post">
				...
				<fieldset>
					...
					<span class="tl select_box">
						<select class="select_style_0" name="searchCnd" id="searchCnd" title="검색 구분을 선택해 주세요.">
					...
					<span class="tl input_search_con">
						...
						<input type="submit" class="search_summit" id="input2" name="input2" value="검색" class="btn btn-primary" />
					</span>
...
<div class="bdList">
	...
	<c:url var="url" value="/${paramVO.siteName }/bbs/${paramVO.bbsId}/view.do?nttId=${result.nttId}&amp;${pageQueryString}" />
		<li><a href="<c:out value='${url}' escapeXml='false'/>" class="db"><span class="title db"><c:out value="${result.nttSj }" /></span>
			<span class="info1">게시일 : <span class="date"> <c:out value="${result.frstRegisterPnttm }"></c:out> </span></span>
