	            ...
	            <td colspan="3">
	                <c:choose>
                        <c:when test="${not empty result.hmpg }">
                            <a href="http://<c:out value="${result.hmpg }"  />" target="_blank" title="새창 열림" ><c:out value="${result.hmpg }"/></a>
                        </c:when>
                        <c:otherwise>
                        -
                        </c:otherwise>
...
<div class="mb20 sub_board_body">
	<table class="board_type_0 small" style="margin-bottom:0;">
		<caption>(콘텐츠아카데미) 모집요강 및 자격 요건 목록 인원,근무지,근무형태,경력,학력,성별로 구분되는 표</caption>
		<thead>
			<tr>
				<th scope="col">인원</th>
				<th scope="col">근무지</th>
...
<div class="mb20 alert">
	...
	<c:forEach var="file" items="${fileList}" varStatus="status">
		<a href="/edumobile/cmm/fms/FileDown.do?atchFileId=${file.atchFileId}&amp;fileSn=${file.fileSn}&amp;bbsId=${param.bbsId}" class="${icn}">
...
<div class="mb30 sub_board_body">
	<table class="contents_insight_view contact_us_view" style="word-break:break-all;">
		<caption>(콘텐츠아카데미) 담당자정보 상세</caption>
		<colgroup>
		<col style="width:20%"  />
		<col/>
		<col style="width:20%"  />
...
<c:url var="url" value="/edumobile/job/${paramVO.programId}/list.do?"/>
