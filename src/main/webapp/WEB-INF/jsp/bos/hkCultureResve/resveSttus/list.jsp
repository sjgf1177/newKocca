...
$(function(){
	...
	$("#excelDownBtn").click(function() {
		...
		if (confirm("엑셀을 다운로드 하시겠습니까?")) {
			$("#resveSttusForm").attr("action","/bos/hkCultureResve/resveSttus/downloadExcel.xls");
			$("#resveSttusForm").submit();
			$("#resveSttusForm").attr("action","/bos/hkCultureResve/resveSttus/list.do");
		}
		return false;
	...
	$("#useAt_chg_exec").click(function() {
		...
		var url = "/bos/hkCultureResve/resveSttus/chgResveSn.json";
		var params = {
...
function fnSpceNmList(spceNm) {
	var url = "/bos/hkCultureResve/resveSttus/listFcltySpceNm.json";
	var params = {
		areaSe : $("#areaSe").val(),
		lcSe : $("#lcSe").val(),
		fcltySe : $("#fcltySe").val()
...
<form id="resveSttusForm" name="resveSttusForm" method="get" action="/bos/hkCultureResve/resveSttus/list.do">
			...
			<table class="table">
				...
				<tbody>
					...
					<tr>
						...
						<th scope="row"><label for="resveNm">단체명</label></th>
						<td><input type="text" name="resveNm" id="resveNm" value="<c:out value="${param.resveNm }" />" /></td>
						<th scope="row" ><label for="mbtlnum">연락처</label></th>
	...
	<table class="table table-bordered table-striped table-hover">
		...
		<c:forEach var="result" items="${resultList}" varStatus="status">
			<tr>
				...
				<td><a href="/bos/hkCultureResve/resveSttus/view.do?resveSn=<c:out value="${result.resveSn }" />&amp;${pageQueryString}"><c:out value="${result.spceNm }" /> <c:if test="${not empty result.spceNmEn }"></c:if></a></td>
