...
$(function() {
	...
	$("#prevBtn").click(function() {
		$(".reservSysSet").fadeOut("slow");
		return false;
		//window.location.href = "/<c:out value="${paramVO.siteName}" />/hkFcltyResve/resveSttus/list.do?menuNo=<c:out value="${param.menuNo}" />";
		//return false;
	});

	fnAreaSeList($("#areaSeDiv"), codeIdVal,0,areaSeVal);  //COM144
...
function fnSpceNmList(obj, pAreaSe, pLcSe, pFcltySe) {
	...
	var $obj = obj;
	var url = "/<c:out value="${paramVO.siteName}" />/hkCultureResve/resveSttus/listFcltySpceNm.json";
	var params = {
		areaSe : pAreaSe,
		lcSe : pLcSe,
		fcltySe : pFcltySe
		...
		if (data) {
			...
			$.each(data.resultList, function(key,item) {
				...
				var $btnItem = $("<a>").attr({
					...
					'href': "/ckl/hkFcltyResve/fclty/viewPopup.do?fcltySn="+item.fcltySn + "&viewType=BODY",
...
function fnLoadCanlendar(obj, pFcltySn, pYear, pMonth, pDay, chkSum) {
	...
	var $obj = obj;
	$obj.empty();
	var url = "/<c:out value="${paramVO.siteName}" />/hkCultureResve/resveSttus/listCal.do";
	var params = {
		fcltySn : pFcltySn,
		year  : pYear,
		month : pMonth,
...
function fnResveSave(resveTp) {
	...
	var url = "/bos/hkCultureResve/resveSttus/resveStep2.do";
...
<div class="reservSysSet">
	<div class="reservSys" style="width:1005px;">
