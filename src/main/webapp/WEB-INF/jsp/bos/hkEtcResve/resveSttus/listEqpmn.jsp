...
<c:forEach var="result" items="${list }" varStatus="status">
	...
	<li style="display: inline-table;width: 100%;" ><label for="eqpmnSn${status.index }"><input type="checkbox" name="eqpmnSn" id="eqpmnSn${status.index }" value="<c:out value="${result.eqpmnSn }" />" /> <c:out value="${result.eqpmnNm }" /></label>
		<%-- <a class="" href="/ckl/hkEtcResve/eqpmn/viewPopup.do?eqpmnSn=<c:out value="${result.eqpmnSn }"/>&amp;viewType=BODY" onclick="window.open(this.href,'eqpmnPop','width=800, height=925, scrollbars=no');return false;" class="text-danger">[장비보기]</a> --%>
		<div>
