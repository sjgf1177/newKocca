...
<p><label for="noEqpnm"><input type="checkbox" name="noEqpnm" id="noEqpnm" value="N" <c:if test="${fn:length(eqpmnList) == 0 }">checked="checked"</c:if> />장비 이용 안함</label></p>

<ul id="eqpmnList" <c:if test="${fn:length(eqpmnList) == 0 }">style="display:none;"</c:if>>
<c:forEach var="result" items="${list }" varStatus="status">
	<li><label for="eqpmnSn${status.index }"><input type="checkbox" name="eqpmnSn" id="eqpmnSn${status.index }" value="<c:out value="${result.eqpmnSn }" />" /> <c:out value="${result.eqpmnNm }" /></label>
