	...
	function delFile2(atchFileId, fileSn, bbsId, fileId){
		$.getJSON(
				...
				function(data)
				{
		            ...
		            if( jdata == 'success' ) {
		            	alert("성공적으로 삭제하였습니다.");
		            	$("#"+fileId).html("<input type=\"file\" name=\""+fileId+"\" id=\""+fileId+"\" class=\"input_file form-control\"/> 이미지 권장사이즈 : 260*220");
		            }
		...
		<div class="bdView">
			<table summary="<c:if test="${roleSuper eq 'Y'}">게시구분,게시기간,</c:if>제목,담당부서,내용<c:if test="${not empty result}">등록일,</c:if><c:if test="${not empty result}">조회수,</c:if> 포함">
				...
				<tbody>
					...
					<tr>
						<th scope="row"><label for="option1">플랫폼 기관</label></th>
						<td>
						<select name="option1"  title="소속">
								<option value="">소속 선택</option>
								<c:forEach var="code" items="${psitnCodeList }">
									<option value="${code.insttSn}" <c:if test="${result.option1 eq code.insttSn }">selected="selected"</c:if>>
					...
					<tr>
						<th scope="row"><label for="option2">멘티 & 멘토 선택</label></th>
						<td>
						<c:forEach items="${COM089CodeList }" var="code" varStatus="status">
								<label> <input type="radio" name="option2"  value="${code.code }" id="option2Nm<c:out value="${status.count }" />" name="option" <c:if test="${result.option2 eq code.code }">checked="checked"</c:if> />${code.codeNm }
								</label>
						</c:forEach>
							</td>
					</tr>
