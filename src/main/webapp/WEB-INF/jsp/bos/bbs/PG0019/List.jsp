	...
	<div id="content">
		...
		<div class="sh">
			<ul>
				...
				<select name="option1" id="option1" onchange="window.location.href=this.value;">
					...
					<c:forEach var="code" items="${COM101CodeList}" varStatus="status">
						<option value="list.do?option1=<c:out value='${code.code }'/>&amp;menuNo=<c:out value='${paramVO.menuNo }'/>" <c:if test="${code.code eq paramVO.option1}">selected="selected"</c:if>>${code.codeNm}</option>
					</c:forEach>
				</select>
			</ul>
		</div>
