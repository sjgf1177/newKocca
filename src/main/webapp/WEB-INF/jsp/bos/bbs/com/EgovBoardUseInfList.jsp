...
<script type="text/javascript">
	...
	function fn_egov_select_bbsUseInfs(pageNo){
		...
		document.frm.action = "<c:url value='/admin/bbs/com/selectBBSUseInfs.do'/>";
		document.frm.submit();
	}
	function fn_egov_insert_addbbsUseInf(){
		document.frm.action = "<c:url value='/admin/bbs/com/addBBSUseInf.do'/>";
		document.frm.submit();		
	}
	function fn_egov_select_bbsUseInf(bbsId, trgetId){
		document.frm.bbsId.value = bbsId;
		document.frm.trgetId.value = trgetId;
		document.frm.action = "<c:url value='/admin/bbs/com/selectBBSUseInf.do'/>";
		document.frm.submit();
	}
	...
	<table width="100%" cellpadding="8" class="table-line">
	 <thead>
	  <tr>
	    ...
	    <th class="title" width="30%" nowrap>게시판명</th>
	    <th class="title" width="20%" nowrap>사용 커뮤니티 명</th>
	    <th class="title" width="15%" nowrap>사용 동호회 명</th>   
	    <th class="title" width="15%" nowrap>등록일시</th>         
	    <th class="title" width="7%" nowrap>사용여부</th>   
	  </tr>
	 </thead>    
	 <tbody>
	 <c:forEach var="result" items="${resultList}" varStatus="status">
	    ...
	    <a href="javascript:fn_egov_select_bbsUseInf('<c:out value="${result.bbsId}"/>','<c:out value="${result.trgetId}"/>');"><c:out value="${result.bbsNm}"/></a>
	    </td>
