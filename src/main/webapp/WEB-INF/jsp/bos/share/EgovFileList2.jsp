		...
		<li>
			...
			<c:choose>
				<c:when test="${updateFlag=='Y' or not empty result.nttId}">
					...
					<c:if test="${fileVO.fileFieldName ne 'thumbImage' and !fn:startsWith(fileVO.fileFieldName,'file_')}">
					<a href="/bos/cmm/fms/FileDown.do?atchFileId=${fileVO.atchFileId}&amp;fileSn=${fileVO.fileSn}&amp;bbsId=${param.bbsId}"  class="${icn}">
						<c:out value="${fileVO.orignlFileNm}"/>&nbsp;[<c:out value="${fileVO.fileMg}"/>&nbsp;byte]
					</a>
					</c:if>
					
					<c:if test="${(paramVO.siteName ne 'portal' and fileVO.fileFieldName ne 'thumbImage' and !fn:startsWith(fileVO.fileFieldName,'file_')) or param.updateFlag=='Y'}">
						<a href="#" onclick="javascript:delFile('${fileVO.atchFileId}', '${fileVO.fileSn}', '${param.bbsId}');">
							<img src="<c:url value='/bos/images/btn_X.jpg'/>" alt="<c:out value="${fileVO.orignlFileNm}"/> 삭제" />
						</a>
					</c:if>
