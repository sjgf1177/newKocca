...
$(document).ready(function(){
	...
	var strKey = '${paramVO.searchWrd}'; // 하이라이트를 적용할 스트링
	 ...
	 if(strKey != ''){
		...
		<!-- 게시판 게시물검색 start -->
	<form  id="frm" name="frm" action="/bos/ntcnAlert/list.do?menuNo=${param.menuNo}" method="post">
		<div class="sh">
			<fieldset>
				...
				<c:forEach items="${resultList}" var="item" varStatus="status">
					<tr>
						...
						<c:if test="${not empty item.fileSn}">
							<img src="/cmm/fms/getImage.do?atchFileId=${item.atchFileId}&fileSn=${item.fileSn}" style="border: 0px;" width="126" height="73" onerror="this.src='/portal/images/common/noimage-error.png'"/>
						</c:if>
						</td>
						<!-- 알림 URL
						<td class="tal">
						...
						<td>
							<c:choose>
								<c:when test="${item.useAt eq 'Y'}"><font color="blue">사용</font></c:when>
...
<!-- board list end //-->
	...
	<div class="btn_set">
		<a class="btn btn-primary" href="/bos/main/removeCache.do"><span>메인캐시삭제</span></a>
		<a class="btn btn-primary" href="/bos/ntcnAlert/forInsert.do?type=01&menuNo=${param.menuNo}"><span>등록</span></a>
	</div>
