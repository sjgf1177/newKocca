...
$(function() {
	...
	$("#schdulBtn").click(function() {
		if(confirm("일정으로 등록하시겠습니까?")) {
			var url = "/bos/fcltyResve/schdul/insert.json";
			var params = {
				eventDt : $("#resveDt").val(),
	...
	$("#cmmntSaveBtn").click(function() {
		...
		var url = "/bos/fcltyResve/resveSttus/saveCmmnt.json";
		var param = {
...
function fnConfirmProcess(resveSttus) {
	var url = "/bos/fcltyResve/resveSttus/updateResveSttus.json";
	params = {
...
<div class="row">
	...
	<c:if test="${result.resveSttus eq '03' }">
		<a href="/bos/fcltyResve/resveSttus/resveStep1.do?resveSn=<c:out value="${result.resveSn }" />&menuNo=<c:out value="${param.menuNo }" />" id="waitUpdBtn" class="btn btn-danger">대기예약정보변경</a>
	</c:if>
	</div>
	...
	<div class="col-md-2"><a class="btn btn-primary" href="/bos/fcltyResve/resveSttus/list.do?${pageQueryString }"><span>목록</span></a></div>
</div>
