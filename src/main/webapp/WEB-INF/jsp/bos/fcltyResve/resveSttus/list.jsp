...
$(function(){
	...
	$("#excelDownBtn").click(function() {
		...
		if (confirm("엑셀을 다운로드 하시겠습니까?")) {
			$("#resveSttusForm").attr("action","/bos/fcltyResve/resveSttus/downloadExcel.xls");
			$("#resveSttusForm").submit();
			$("#resveSttusForm").attr("action","/bos/fcltyResve/resveSttus/list.do");
		}
		return false;
...
<form id="resveSttusForm" name="resveSttusForm" method="get" action="/bos/fcltyResve/resveSttus/list.do">
	...
	<table class="table table-bordered table-striped table-hover">
		...
		<c:forEach var="result" items="${resultList}" varStatus="status">
			<tr>
				...
				<td><a href="/bos/fcltyResve/resveSttus/view.do?resveSn=<c:out value="${result.resveSn }" />&amp;${pageQueryString}"><c:out value="${result.spceNm }" /> <c:if test="${not empty result.spceNmEn }"></c:if></a></td>
...
<!--
<a class="btn btn-primary" href="/bos/fcltyResve/resveSttus/resveStep1.do?menuNo=<c:out value="${param.menuNo }" />"><span>예약하기</span></a>
 -->
 </div>
