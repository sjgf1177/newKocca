...
<div id="content">
	...
	<form id="board" name="board" method="post" enctype="multipart/form-data" action="${action}">
		<input type="hidden" name="pageQueryString" value="${pageQueryString}" /> <input type="hidden" name="cnslSn" value="${empty result.cnslSn ? 0 : result.cnslSn }" /> <input type="hidden" name="htmlYn" value="${result.htmlYn}" /> <input type="hidden" id="cnsltReqSe" name="cnsltReqSe" /> <input type="hidden" name="frstRegisterId" value="<c:out value="frstRegisterId" />"> <input type="hidden" name=lastUpdusrId value="<c:out value="lastUpdusrId" />">

		<h4>■ 상담자정보</h4>
		<div class="bdView">
			<table>
				<caption>상담자정보</caption>
				<colgroup>
					<col style="width: 15%" />
					<col style="width: 35%" />
					<col style="width: 15%" />
				...
				<tbody>
					<tr>
						...
						<td colspan="3"><input type="radio" name="option1" value="0" <c:if test="${result.option1 eq '0' }"> checked="checked" </c:if> />직접입력 <input type="radio" name="option1" value="1" <c:if test="${result.option1 eq '1' }"> checked="checked" </c:if> />회원선택 <br /> <input type="text" name="userNm" value="${result.userNm }" /></td>
					</tr>
					<tr>
						<th scope="row">성별</th>
						<td><c:forEach var="code" items="${COM079CodeList}" varStatus="status">
						...
						<td><input type="text" id="brthdy" name="brthdy" readonly="readonly" value="${result.brthdy}" class="board1 calendar" title="생년월일" /></td>
					</tr>
					<tr>
						<th scope="row">학교</th>
						<td><input type="text" name="schul" id="schul" class="" value="${result.schul}" /></td>
						<th scope="row">학과</th>
						<td><input type="text" name="subjct" id="subjct" class="" value="${result.subjct}" /></td>
					</tr>
					<tr>
						<th scope="row">자택</th>
						<td><c:set var="data" value="${fn:split(result.ownhomTelno,'-')}" /> <c:choose>
							...
							</c:choose> <c:set var="data" value="" /></td>
						<th scope="row">휴대폰</th>
						<td><c:set var="data" value="${fn:split(result.mbtlnum,'-')}" /> <c:choose>
								<c:when test="${fn:length(data) >= 3}">
					...
					<tr>
						<th scope="row">이메일</th>
						<td colspan="3"><c:set var="data" value="${fn:split(result.email,'@')}" /> <c:choose>
						...
						</script> <c:set var="data" value="" /></td>
					</tr>
					<tr>
						<th scope="row">주소</th>
		...
		<div class="bdView">
			<table>
				...
				<tbody>
					...
					<tr>
						<td class="outputEditor" colspan="4"><span style="color: red;">* 아래의 작성하시는 내용 (첨부파일 포함)중에 개인정보에 관한 내용이 있다면 개인정보가 타인에게 노출되어 침해 받을 수 있으므로 삭제 후 등록하여 주시기 바랍니다. (개인정보내용 : 주민등록번호, 계좌번호, 핸드폰번호 등) </span><br /> <textarea id="cnsltReqCn" name="cnsltReqCn" cols="150" rows="30" style="display: none;" class="textarea">${result.cnsltReqCn}</textarea> <script type="text/javascript">
						//<![CDATA[
