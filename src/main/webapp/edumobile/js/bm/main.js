/*var mainRollingObj = {
	...
	,init : function(){
		...
		$('.caroufredsel_arrow').on('click', function(){
			_this.setRollingEvent($(this), true );
//			_this.setRollingEvent($(this) );
		});
		this.setResizeEvent();
		_this.$itemList.attr('aria-hidden', 'true');
		$($('#mainSubRolling .item')[_this.selectIdx]).attr('aria-hidden', 'false');
	...
	, setClearInterval : function(){
		//$('.caroufredsel_arrow').css("pointer-events","none"); //rsg20180111
		clearInterval(this.autoPlay);
	}

	, setIntervalEvent : function(){
		//$('.caroufredsel_arrow').css("pointer-events",""); //rsg20180111
		var _this = this;
		this.autoPlay = setInterval(function(){
			_this.setRollingEvent($('.caroufredsel_arrow.right'), false);
//			_this.setRollingEvent($('.caroufredsel_arrow.right'));
		}, 6000);
	}
	, setRollingEvent : function(el, click){
		if(this.isRolling) return;
		var _this = this;
		this.isRolling = true;
		//this.setClearInterval(); //rsg20180116
		this.$itemList = $('#mainSubRolling .item');
		...
		if(el.hasClass('right')){
			...
			this.$firEl.attr('aria-hidden', 'true');
			this.$conEl.animate({'left':(-300)+"px"}, 800, function(){
				...
				//_this.setIntervalEvent(); //rsg20180116
				_this.isRolling = false;
			});
		}else{
			this.$conEl.prepend(this.$lstEl);
			this.$lstEl.attr('aria-hidden', 'false');
...
function onAttrChange(){
	$('.event_body_con.swiper-container .swiper-slide').attr('aria-hidden', 'true');
	$('.event_body_con.swiper-container .swiper-slide-active').attr('aria-hidden', 'false');
}

function onAttrChangeModi(idx) {
	var slideList = $('.contents_body.swiper-wrapper .swiper-slide');
	var trueSlideIdxList = [idx + 1, idx + 2, idx + 3, idx + 4];
	
	for(var i = 0, len = slideList.length; i < len; i++) {
		var swiperSlide = $(slideList[i]);
		if(trueSlideIdxList.indexOf(i) != -1) swiperSlide.attr('aria-hidden', 'false');
		else swiperSlide.attr('aria-hidden', 'true');
	}
}
...
window.onload = function(){
	...
	swiper0 = new Swiper('#event_body_con', {
		...
		,nextButton: '#event_body_con .swiper-button-next'
		,prevButton: '#event_body_con .swiper-button-prev'
		// 더 외 추가 가능 요소들
...
//	        var i = geSlideDataIndex(swiper);
	...
	swiper5 = new Swiper('#careers_body_con', {
		...
		,mousewheelControl: true // 마우스 휠로 슬라이드 움직임
		,autoplayDisableOnInteraction: false // 인터렉션 이후 다시 오토 플레이
		,nextButton: '#careers_body_con .swiper-button-next'
		,prevButton: '#careers_body_con .swiper-button-prev'
	    ...
	    ,onSlideChangeStart : function(swiper){
	    	var idx = geSlideDataIndex(swiper);
	    	var slideList = $('.contents_body.swiper-wrapper .swiper-slide');
	    	var swiperSlide = $(slideList[idx + 1]);
	    	
	    	onAttrChangeModi(idx);
	    }
	});
	/*
	...
	*/
	function setPagination(){
		if($(window).width() > 992){
			if ( typeof swiper4.unlockSwipes === 'undefined' )return;
			swiper4.lockSwipes();
    ...
    var mainVisualAutoPlay = setInterval(visualRight, 3000);
    ...
    $(".event_body_con .rolling_stop_btn").on('click', function(){
        swiper0.stopAutoplay();
    });

    $(".event_body_con .rolling_start_btn").on('click', function(){
    	swiper0.startAutoplay();
    });

    $(".careers_body_con .rolling_stop_btn").on('click', function(){
    	swiper5.stopAutoplay();
    });

    $(".careers_body_con .rolling_start_btn").on('click', function(){
